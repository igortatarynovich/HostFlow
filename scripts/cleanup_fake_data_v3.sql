-- Скрипт для очистки фейковых данных (версия 3 - удаляет service_orders вместо обновления)
...
BEGIN
    ...
    -- Удаляем связанные данные фейковых кандидатов
    
    -- 1. Service order items и schedules
    DELETE FROM service_attachments WHERE service_order_id IN (
        SELECT id FROM service_orders WHERE candidate_id IN (
    ...
    DELETE FROM service_schedule WHERE service_order_id IN (
        SELECT id FROM service_orders WHERE candidate_id IN (
            SELECT id FROM candidates WHERE deleted_at IS NULL AND (
                first_name IN ('Test', 'Boris', 'Anna', 'Bulk', 'Order', 'Auto', 'Lead', 'Owner', 'Final', 'Existing', 'Manual', 'Candidate', 'Draft', 'Flow', 'Assigned', 'Driver', 'Fallback', 'Stage')
                OR last_name IN ('Declined', 'Awaiting', 'Candidate', 'Flow', 'Assigned', 'Driver', 'Fallback', 'Stage', 'Draft')
