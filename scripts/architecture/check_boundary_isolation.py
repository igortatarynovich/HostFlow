#!/usr/bin/env python3
"""PMI-B: foreign modules may import Boundary only via public contracts.

Named foreign owners importing Boundary internals (non-public) must be on the
shrink-only exception allowlist. See docs/modules/boundary/module_isolation_card.md.
"""
...
ALLOWLIST_PATH = (
    REPO_ROOT / "scripts" / "architecture" / "boundary_isolation_allowlist.txt"
)
PUBLIC_PREFIX = "backend/app/modules/boundary/public"
FOREIGN_OWNERS = {
    "recruitment",
    "employment",
    "documents",
...
def is_public(to_file: str, import_mod: str) -> bool:
    if to_file == PUBLIC_PREFIX or to_file.startswith(PUBLIC_PREFIX + "/"):
        return True
    if "modules.boundary.public" in import_mod:
        return True
    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_allowlist:
        lines = [
            "# PMI-B boundary isolation allowlist (shrink-only).",
            "# Format: owner|from_file::import::to_file",
            "# New rows FORBIDDEN after PMI-B PASS.",
            "# Outbound Boundary→Employment/Documents internals classified on Isolation Card",
            "# (EXC-PMI-B-EMP / EXC-PMI-B-DOC) — not enforced by this inbound scanner.",
        ]
    ...
    if new_ids:
        print("PMI-B boundary isolation STOP: new non-public Boundary imports", file=sys.stderr)
    ...
    if args.json:
        ...
        print("PMI-B boundary isolation")
        print(f"  foreign→boundary non-public (allowlisted): {len(ids)}")
        print("  new violations: 0")
        print("  rule: named foreign owners → boundary.public only (exceptions shrink-only)")
    return 0
