#!/usr/bin/env python3
"""PMI-W: foreign modules may import Workforce only via public contracts."""
...
ALLOWLIST_PATH = REPO_ROOT / "scripts" / "architecture" / "workforce_isolation_allowlist.txt"
PUBLIC_PREFIX = "backend/app/modules/workforce/public"
FOREIGN_OWNERS = {
    "recruitment",
    "boundary",
    "employment",
...
def is_public(to_file: str, import_mod: str) -> bool:
    if to_file == PUBLIC_PREFIX or to_file.startswith(PUBLIC_PREFIX + "/"):
        return True
    if "modules.workforce.public" in import_mod:
        return True
    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_allowlist:
        lines = [
            "# PMI-W workforce isolation allowlist (shrink-only).",
            "# Format: owner|from_file::import::to_file",
            "# New rows FORBIDDEN after PMI-W PASS.",
            "# Workforce accepts Started Employee continuity; does not reconstruct Ready/RFE/Admit.",
        ]
    ...
    if new_ids:
        print("PMI-W workforce isolation STOP: new non-public Workforce imports", file=sys.stderr)
    ...
    if args.json:
        ...
        print("PMI-W workforce isolation")
        print(f"  foreign→workforce non-public (allowlisted): {len(ids)}")
