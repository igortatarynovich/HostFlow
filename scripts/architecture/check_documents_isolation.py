#!/usr/bin/env python3
"""PMI-D: foreign modules may import Documents only via public evidence contracts."""
...
ALLOWLIST_PATH = REPO_ROOT / "scripts" / "architecture" / "documents_isolation_allowlist.txt"
PUBLIC_PREFIX = "backend/app/modules/documents/public"
FOREIGN_OWNERS = {
    "recruitment",
    "boundary",
    "employment",
...
def is_public(to_file: str, import_mod: str) -> bool:
    if to_file == PUBLIC_PREFIX or to_file.startswith(PUBLIC_PREFIX + "/"):
        return True
    if "modules.documents.public" in import_mod:
        return True
    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_allowlist:
        lines = [
            "# PMI-D documents isolation allowlist (shrink-only).",
            "# Format: owner|from_file::import::to_file",
            "# New rows FORBIDDEN after PMI-D PASS.",
            "# Documents publishes evidence facts only — not Ready/Admit/Workforce lifecycle.",
        ]
    ...
    if new_ids:
        print("PMI-D documents isolation STOP: new non-public Documents imports", file=sys.stderr)
    ...
    if args.json:
        ...
        print("PMI-D documents isolation")
        print(f"  foreign→documents non-public (allowlisted): {len(ids)}")
