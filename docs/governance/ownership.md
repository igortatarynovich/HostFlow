...
| **Architecture canon** | `docs/specs/architecture/hostflow-core-domain-map-v1.md`, `platform-architecture-principles.md`, `module-catalog-and-routing-map.md`, ADR-002…014 | Architecture canon owner | Аппрувит ADR, держит domain map консистентным, защищает bounded contexts |
...
| **Module canon (per module)** | `docs/<module>/module-scope.md` + `docs/specs/modules/<module>.md` | Module owner (recruitment / hr / fleet / services / finance / forms / document-hub) | Аппрувит изменения scope модуля; следит, что module spec не противоречит ADR |
