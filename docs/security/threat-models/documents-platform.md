# Threat Model — Documents Platform (E3 + E4 public resolve)
...
This model covers the **platform capability** consume path sealed in E3 and extended in E4: HR employee (D8) reads reused documents via Document Link (`workforce_employee` / `reused_for_hr`); Candidate (D4) reads via Document Link (`candidate` / `primary`). Neither consumer uses a page-local widget as the D2 path. Candidate D2 consume is **not** `documents.candidate_id`. File bytes, signed URLs, and upload remain [`document-uploads.md`](./document-uploads.md). Candidate portal uploads remain [`candidate-portal.md`](./candidate-portal.md). Handoff copy-vs-link remains [`handoff.md`](./handoff.md) — E3/E4 do not add a file-copy path.

Not this surface: OCR / e-sign / packages, D3 / D5–D7 / D9 bind, `candidate_id` column drop, Foundation close, anonymous public links.
...
- `documents.candidate_id` remains a Candidate storage bridge; it is not the D4 or D8 D2 consume key. E4 may mint Hub primary links from the FK (ensure-on-read); listing still goes through `document_entity_links`
...
| DP-8 | Candidate-id consume for D2 | Using `documents.candidate_id` as the D4 or D8 D2 path |
...
- Same adapter id as E2. Named **Documents Platform E4 Candidate Document Link Gate** fails if D4 binds via CandidateCard / `candidate_id` list, if D8 unbinds, or if D3 / D5–D7 / D9 bind `documents`.  
