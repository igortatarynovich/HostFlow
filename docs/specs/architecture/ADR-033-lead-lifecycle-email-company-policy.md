...
- 2026-09-04: **Errata / restricted transitions** — no universal mark-resolved. `delivery_failed` → `delivered` only after successful SMTP send; `review_required` → `compliant` only with assessment proof (notice at source or operator attestation); `review_required` → `exempt` only with a valid reason code. Webhook/notify is not fulfillment proof.
