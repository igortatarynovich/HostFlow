...
> Operator surface (MA-3): [`mapping-authority-operator.md`](mapping-authority-operator.md) — UX contract; Mapping Operator Gate **PASS**. Consumer cutover (MA-4): feat `feat/mapping-authority-ma4-consumer-cutover` open; Mapping Consumer Cutover Gate not PASS.
