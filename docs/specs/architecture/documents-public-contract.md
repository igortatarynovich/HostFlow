...
**Tasks:** [`documents-platform-e1-contract-seal.md`](../tasks/documents-platform-e1-contract-seal.md) ✅ · [`documents-platform-e2-public-contract.md`](../tasks/documents-platform-e2-public-contract.md) ✅ · [`documents-platform-e3-first-consumer-bind.md`](../tasks/documents-platform-e3-first-consumer-bind.md) (feat)  
...
Storage bridge: existing `document_hub_delivery_contract.py` façade over `modules.documents`. E3 adds **entity-link resolve** on the same adapter for HR employee (`workforce_employee` / `reused_for_hr`) via Hub table `document_entity_links`. `documents.candidate_id` remains a legacy bridge until a later Candidate bind.
...
|----|-----------|--------------------------|-------------|
| `list` / `resolve` | **Stable** | `list_candidate_documents_via_contract` · `list_entity_link_documents_via_contract` | Entity-scoped read. Candidate input stays the legacy bridge. E3 D8 consume path is entity-link resolve (`workforce_employee` / `reused_for_hr`). Output is Hub document view, not a module file row |
...
4. D2 `documents` slot on D8 renders via this adapter only — not Shell nav, not dossier pages, not `HrEmployeeDocumentsSection`.  
...
| **Document Link SoT** | E3 first consumer (HR employee) via `document_entity_links`. `documents.candidate_id` remains the Candidate storage bridge |

---

## D2 catalog unlock + first consumer bind

E2 marked D2 `documents` as an **enabled platform slot**. E3 binds it on **HR employee (D8)** through the Capability Host contribution. D3–D7 / D9 stay unbound. Catalog unlock ≠ mass bind. Shell `EntityWorkspaceSectionId` `documents` ≠ this slot.
...
`backend/tests/platform/test_documents_e3_first_consumer_bind_gate.py` — named **Documents Platform E3 First Consumer Bind Gate**. E1 / E2 / D1–D9 / WCP gates stay green (amended where they froze “no consumer bind”).
