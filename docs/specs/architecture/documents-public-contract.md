...
|----|-----------|--------------------------|-------------|
| `list` / `resolve` | **Stable** | `list_candidate_documents_via_contract` · `list_entity_link_documents_via_contract` | Entity-scoped read. E3 D8 consume path is entity-link resolve (`workforce_employee` / `reused_for_hr`). E4 D4 consume path is entity-link resolve (`candidate` / `primary`). `list_candidate_documents_via_contract` stays the legacy bridge, not the D4 D2 path. Output is Hub document view, not a module file row |
...
E2 marked D2 `documents` as an **enabled platform slot**. E3 binds it on **HR employee (D8)**. E4 binds it on **Candidate (D4)** through Document Link. D3 / D5–D7 / D9 stay unbound. Catalog unlock ≠ mass bind. Shell `EntityWorkspaceSectionId` `documents` ≠ this slot.
