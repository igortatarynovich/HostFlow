...
Creating an Employee is **forbidden** unless `ready_to_create_employee=true` (this contract). ESO-4 reference evaluate emits the allow signal only.

**ESA2 integration (runtime orchestrator):** on **authoritative Formalize apply/complete** with `ready_to_create_employee=true`, HostFlow composes existing `ensure_employee_after_formalize_apply` → `handoff_from_candidate` (handoff-linked). Formalize **evaluate/read** must not mint. This is not an HR employee card and not ESO-5 Started.
...
| Emit `ready_to_create_employee` when complete | Create HR card / Started / mint on evaluate |
| Compose ESA2 ensure mint **only** on authoritative apply + ready | Invent a second mint authority |
...
- Minting on Formalize evaluate/read (write side effect).  
...
**PASS** when this document + `employment_formalize.v1` + gate tests exist, the ESO brief names Formalize, and HostFlow can answer **formalization_complete / blocked / missing** with context-derived required actions and emit **`ready_to_create_employee` only when complete** — without a universal checklist. Authoritative apply may mint via ESA2 ensure; evaluate/read must not.
