...
**Amended by [`ADR-042`](ADR-042-operator-host-boundary.md):** the operator **host** after handoff is **HR**, not the Recruitment Application card. Fits on an Application enters Candidates; it is not Recruitment complete. Transfer lives on the Candidate after Ready for employment. Formalize / Started are HR-owned. This ADR’s ownership split is unchanged; only the mistaken “one card forever” hosting is withdrawn.
...
- [`ADR-042-operator-host-boundary.md`](ADR-042-operator-host-boundary.md) — Application / Candidate / HR operator hosts; seamless handoff ≠ same screen
