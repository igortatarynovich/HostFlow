...
- **Entity Workspace:** Phase D D9 brief ([Services order cutover](../tasks/entity-workspace-d9-services-order-cutover.md); feat locked). D8 ✅ ([#266](https://github.com/igortatarynovich/HostFlow/pull/266)). Next = Documents Phase E (locked). Shell adapter on tip; Sales Inquiry + Candidate + Client + Sales Order + Vacancy + HR employee bound; Services order is this consumer; Shell nav ≠ D2 slots; `documents` reserved empty; Universal runtime not yet. Foundation 🔄 ([A2-F7](../gates/platform-governance-review-a2.md)).  
