...
- **Entity Workspace:** Phase D D8 feat ([HR employee cutover](../tasks/entity-workspace-d8-hr-employee-cutover.md); named Cutover Gate). D7 ✅ ([#264](https://github.com/igortatarynovich/HostFlow/pull/264)). Next = D9 brief (locked). Shell adapter on tip; Sales Inquiry + Candidate + Client + Sales Order + Vacancy + HR employee bound; Shell nav ≠ D2 slots; `documents` reserved empty; Universal runtime not yet. Foundation 🔄 ([A2-F7](../gates/platform-governance-review-a2.md)).  
