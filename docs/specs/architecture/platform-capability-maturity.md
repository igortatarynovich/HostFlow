...
- **Entity Workspace:** Phase D D6 feat ([sales order cutover](../tasks/entity-workspace-d6-sales-order-cutover.md); named Cutover Gate). D5 ✅ ([#260](https://github.com/igortatarynovich/HostFlow/pull/260)). Next = D7 brief (locked). Shell adapter on tip; Sales Inquiry + Candidate + Client + Sales Order bound; Shell nav ≠ D2 slots; `documents` reserved empty; Universal runtime not yet. Foundation 🔄 ([A2-F7](../gates/platform-governance-review-a2.md)).  
