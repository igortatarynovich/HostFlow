...
- 2026-09-03: **ADR-041** (Accepted; runtime not started) — verified Growth signup: SignupIntent pre-tenant; `intent` → `verify` → `complete` → `resend`; trial starts only on atomic User+Tenant+membership+TenantLicense(`plan=trial`)+OwnCompany; legacy `/auth/register` retired at cutover — [`ADR-041-verified-self-service-signup.md`](ADR-041-verified-self-service-signup.md). Threat model: [`../../security/threat-models/verified-self-service-signup.md`](../../security/threat-models/verified-self-service-signup.md).
