...
| **Public intake (P5C Lead-first)** — `POST /api/v1/public/intake` | `create_or_reuse_public_intake_lead_draft()` — **Lead only** on create; Candidate on submit via `submit_public_intake_lead_draft()` → Decision Layer → `execute_outcome_decision()` | **No** on create; **Yes** on submit outcome | Да (outcome path) | Низкий для нового traffic — см. [ingestion-contract-public-intake.md](ingestion-contract-public-intake.md) |
| **Public intake (legacy Candidate draft)** — `api/public/intake.py` fallback | `resolve_public_intake_session` → legacy `create_public_intake_draft_via_service` → `create_candidate_full` | Да (in-flight tokens only) | Deprecate | Снижается по TTL; не расширять |
