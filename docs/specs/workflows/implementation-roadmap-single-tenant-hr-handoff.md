...
| 2026-07-01 | repo — `feat/documents-runtime-expiry-engine` pytest §6.7 | agent | — | — | да (API) | **§6.7 return-to-recruitment consistency:** `test_return_to_recruitment_consistency.py` PASS — lock release после return, `can_edit` restored, mutations снова доступны, repeat handoff без дублей документов/employee. FE: `getHrOperationalContext` + operational context banner в `HrEmployeeDocumentsSection`. |
