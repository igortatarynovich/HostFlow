...
**Related:** [Applications — operating model](../architecture/applications-operating-model.md) (entity boundaries; **no second enum**), [Application creation — MVP](application-creation-mvp.md) (when rows are created), [Recruitment domain model](../architecture/recruitment-domain-model.md) (Lead / Candidate / Application), [Candidate pipeline](candidate_pipeline.md) (Candidate stage — **different** concern), [Slice 4 — Activity continuity guards](slice-4-activity-continuity-guards.md) (Lead→Candidate handoff: suppress duplicate **first-contact** UOS / reminders using lead signals — **does not** define Application status; see that doc §1.1).
