...
**Status:** **Machine ESO-1…5 PASS** — contracts + orchestrators + named CI on tip; **HR UI:** ESO-1 PASS @ `b6735b9b` / `04f31a74`; **ESO-2+3 Decision Surface PASS** @ `fb61a7b1`; **ESO-4 Formalize HR Binding PASS** @ `68762f13`; **ESO-5 Started HR Binding** — **in progress** on `feat/eso5-started-hr-binding` (not PASS). Inventory: [`../../analysis/hr-employment-eso-inventory.md`](../../analysis/hr-employment-eso-inventory.md).  
**Phase class:** product  
...
> Full product spine remains **NOT PASS** until ESO-5 UI gate PASS, then three-host Full Spine Gate (Formalize PASS @ `68762f13`; Started binding in progress).  
> Seamless handoff = prepared Employment case with known context. Not “same screen forever”.  
> **Seamless UX ≠ shared ownership.**  
> **Inventory (L3):** machine ESO-1…5 ready; ESO-1…4 PASS on `/app/hr/handoffs/:id`; ESO-5 Started HR binding in progress ([`../../analysis/hr-employment-eso-inventory.md`](../../analysis/hr-employment-eso-inventory.md)).
...
4. **ESO-5 Started HR Binding** — **in progress** on `feat/eso5-started-hr-binding` (entry = `ready_to_create_employee=true`; human `confirm_physical_start`; terminal on HR handoff host).  
5. New Full Spine Gate (three-host) **only after** ESO-5 PASS — do **not** retune the withdrawn one-card Full Spine Gate.  
6. RSO-2 Transfer remains Recruitment-owned (Candidate, after Ready for employment) and must **not** call Employment accept / employability / resolution / formalize / started. After Transfer the operator continues in HR ([`ADR-042`](../architecture/ADR-042-operator-host-boundary.md)). Do not pursue [#359](https://github.com/igortatarynovich/HostFlow/pull/359) one-card PASS.  
7. **Canonical Facts Completeness** — [`canonical-facts-completeness.md`](canonical-facts-completeness.md): Employment consumers (ESO-1…2 identity/citizenship) stay on one Person/package projection read path; no Employment-local copy of Recruitment facts.
