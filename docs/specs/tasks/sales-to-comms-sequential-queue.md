...
|-------|-------------|------|
| **Product** | **[HE-1](hiring-workflow-e2e.md)** after External Intake program close. Hiring Acceptance Contract Gate **PASS**. SoT: [hiring-acceptance-contract.md](../architecture/hiring-acceptance-contract.md) (`hiring_acceptance.v1`). Do not start HE-2 / min HR / RS-7 in this PR. Not Mapping/RS-3. Not leftover-store deletion. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[HE-1](hiring-workflow-e2e.md)** after External Intake program close. Hiring Acceptance Contract Gate **PASS**. SoT: [hiring-acceptance-contract.md](../architecture/hiring-acceptance-contract.md) (`hiring_acceptance.v1`). min HR remains queued. |
| **Queued Product successor** | **HE-2** Stage authority consumption after Hiring Acceptance Contract Gate PASS — [brief](hiring-workflow-e2e.md). Do not start HE-2 / min HR / leftover-store deletion / RS-3 / RS-7 in this contract-seal PR. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM program = DONE** (Authority / Operator / 3A / 3B / Consumer Cutover `918274d1` + program close). **MA-1 Contract Gate = PASS**. **Mapping Resolution Gate = PASS**. **Mapping Operator Gate = PASS**. **Mapping Consumer Cutover Gate = PASS**. **Mapping program = DONE**. **Forms Publish Contract Gate = PASS**. **Publish Action Gate = PASS**. **Public Serve Gate = PASS**. **Operator Publish Gate = PASS**. **External Intake Acceptance Gate = PASS**. **External Intake program = DONE**. **Hiring Acceptance Contract Gate = PASS**. **Product = HE-1** (contract sealed; HE-2 feat locked; not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. Active Product is **HE-1** after External Intake program close. Hiring Acceptance Contract Gate **PASS**. Unlock ≠ silent schedule of HE-2 / min HR. Do not start HE-2 runtime. Do not start leftover-store deletion. Do not convert FP-5 PASS into Release Acceptance PASS.  
...
**Now:** Product **[HE-1](hiring-workflow-e2e.md)** after External Intake program close. Hiring Acceptance Contract Gate **PASS**. SoT: [hiring-acceptance-contract.md](../architecture/hiring-acceptance-contract.md) (`hiring_acceptance.v1`). External Intake Acceptance Gate **PASS** (`4f454556`). Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. **Not** Mapping/RS-3. **Not** min HR. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
Product:
  CL0 → CL1 → LI-1 → DR1-contract → CL2 → CL3 → CL4 → CL5 → CL6 → CL7 → Vacancy Overlay Contract → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B → rpm-cutover → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1   (this amendment)
```

Program horizon of this amendment: **HE-1** (Hiring Acceptance Contract). External Intake program close is **DONE**. Mapping program close is **DONE**. RPM program is **DONE**. min HR remains unlocked by Hiring close, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

Hiring workflow E2E is **no longer in this table** — HE-1 is Active Product (Hiring Acceptance Contract Gate **PASS**; HE-2 feat locked). A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time.
...
| **46k** | **HE-1** Acceptance contract | [brief](hiring-workflow-e2e.md) · [contract SoT](../architecture/hiring-acceptance-contract.md) | after External Intake program close; Hiring Acceptance Contract Gate **PASS**; HE-2 feat locked |
...
**Do** amend this queue when switching Product Active (this revision: Active Product = **HE-1**; Hiring Acceptance Contract Gate **PASS**; HE-2 feat locked).
