...
| **Queued Launch-ops** | **OL-2** Deploy, migrate & rollback — named successor, **starts in its own PR** (invariant 6), then OL-3…OL-7 — [brief](operate-and-launch.md). The migration blocker (`alembic upgrade heads` fails on a fresh DB, [AGENTS.md](../../../AGENTS.md)) is owned by **OL-2**, not by the pytest background item |
