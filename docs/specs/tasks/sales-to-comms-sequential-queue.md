...
|-------|-------------|------|
| **Product** | **[RPM-3A Parallel authority retirement](requirement-policy-management.md)** after RPM-2 Operator Gate **PASS** [#342](https://github.com/igortatarynovich/HostFlow/pull/342) / `5196ee64`. Feat locked this PR. Then RPM-3B → Consumer Cutover Gate. Not Mapping. Not Hiring E2E. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[RPM-3A Parallel authority retirement](requirement-policy-management.md)** after RPM-2 Operator Gate **PASS** [#342](https://github.com/igortatarynovich/HostFlow/pull/342) / `5196ee64`. Feat locked this PR. Mapping / Hiring remain closed. |
| **Queued Product successor** | **RPM-3B** Consumer parity — then Consumer Cutover Gate — [brief](requirement-policy-management.md). Mapping remains startable **after** RPM program close (not auto-scheduled). The four remaining v1 blockers are **briefed but unscheduled** — see § v1 blocker programs beyond this horizon. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM-1 = DONE** (Authority Gate PASS). **RPM-2 = DONE** (Operator Gate PASS [#342]). **Product = RPM-3A** (not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. This amendment names **RPM-3A** after RPM-2 Operator Gate PASS. Unlock ≠ silent schedule of Mapping / Hiring. Do not start RPM-3A feat in this docs PR. Do not start RPM-3B in the 3A PR.  
...
```text
CL0 → CL1 → LI-1 → DR1-contract → CL2 → CL3 → CL4 → CL5 → CL6 → CL7 → Vacancy Overlay Contract → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B
...
| **P-RPM3A** | [RPM-3A](requirement-policy-management.md) Parallel authority retirement | **Requirement Policy Parallel Authority Retirement Gate** — A `document_policies`, C leftover ruleset writes, J P3B `document_required` only. Feat locked this PR | **Requirement Policy Operator Gate** | RPM-3B |
...
| **P-RPM3** | [RPM-3](requirement-policy-management.md) Consumer cutover close | **Requirement Policy Consumer Cutover Gate** — 3A ∧ 3B PASS; classified consumers read the same merge or are retired; D4 matches operator write | RPM-3A Gate ∧ RPM-3B Gate | RPM program close. Hiring E2E unlocked, **not** scheduled |

**Now:** Product **RPM-3A** (parallel authority retirement; feat locked this PR) after RPM-2 Operator Gate **PASS**. **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** Mapping auto-start. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
| **Serialized-equivalent total** | both tracks share one delivery capacity today | **36–53** |
...
|----------|------|-----------|----------------------|-------------------|
| S1 — recent pace holds | 2.0 slices/day | 18–27 days | 2026-09-17 … 2026-09-26 | 2026-10-01 … 2026-10-17 |
| S2 — runtime-heavy discount | 1.0 slices/day | 36–53 days | 2026-10-05 … 2026-10-22 | 2026-10-19 … 2026-11-12 |
| S3 — infrastructure reality | 0.5 slices/day | 72–106 days | 2026-11-10 … 2026-12-14 | 2026-11-24 … 2027-01-04 |

Computed from 2026-08-30 over **36–53** serialized-equivalent slices, adding a 7-day acceptance-suite execution window and a 7–14-day defect-fix window after RC. **S2 is the planning scenario**: S1 assumes contract-slice velocity survives contact with deploy pipelines, backups and publish runtime, which the [Operate & Launch](operate-and-launch.md) starting-point inventory contradicts.
...
**Prior revisions:** 35–52 slices → RC 2026-10-03 … 2026-10-20 under S2 (2026-08-29); 34–51 → RC 2026-10-01 … 2026-10-18 (2026-08-28); before that 30–44 → RC 2026-09-27 … 2026-10-11. Every increase so far is the gate-prerequisite line, not new product scope.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: RPM-3A; parallel authority retirement; feat locked this PR).  
...
- **Requirement Policy Management** ← **RPM-1 PASS** · **RPM-2 PASS** [#342](https://github.com/igortatarynovich/HostFlow/pull/342) / `5196ee64` (Operator Gate; [requirement-policy-authority.md](../architecture/requirement-policy-authority.md)). Active **RPM-3A** (parallel authority retirement; feat locked this PR) — [brief](requirement-policy-management.md). RPM-3B queued. Mapping not auto-scheduled. Not Hiring E2E. Not CL8.
...
| **45e** | **RPM-3A** Parallel authority retirement | [brief](requirement-policy-management.md) | after RPM-2 Operator Gate; Active; feat locked this PR; then RPM-3B |
...
After CL0: **CL1 → LI-1 → DR1-contract → CL2…CL7 → Vacancy Overlay Contract → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B**. CL1 observes live codes; it does **not** canonize country or document-type identity. **DR1-contract** is not a Field Composition slice. **DR1-runtime** is **PASS** [#313](https://github.com/igortatarynovich/HostFlow/pull/313). **E8-bind** is **PASS** [#321](https://github.com/igortatarynovich/HostFlow/pull/321) / `8246421f`. **E8-eval** is **PASS** [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6`. **RPM-1** Authority Gate **PASS**. **RPM-2** Operator Gate **PASS** [#342](https://github.com/igortatarynovich/HostFlow/pull/342). Product = **[RPM-3A](requirement-policy-management.md)** (parallel authority retirement; feat locked this PR). **CL7** is Engine evaluation, not Engine→Request. Vacancy Overlay leftover of the original CL0 chain is **PASS** — **not** CL8.
...
**Do not** park later Product on Engineering DONE. Product is **RPM-3A** after RPM-2 Operator Gate PASS (feat locked this PR).  
...
**Do** amend this queue when switching Product Active (this revision: live = **RPM-3A**; parallel authority retirement; feat locked this PR; RPM-2 Operator Gate PASS).
