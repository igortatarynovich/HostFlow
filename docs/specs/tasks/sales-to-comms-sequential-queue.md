...
|-------|-------------|------|
| **Product** | **[HE-4](hiring-workflow-e2e.md)** after Eligibility Composition Gate PASS. Feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`. Hiring E2E Acceptance Gate **not PASS**. This amendment does not execute RS-7. min HR remains queued (feat locked; not scheduled). Not Hiring E2E program close. Not inherited DR1 / Mapping fixes. Not Mapping/RS-3. Not leftover-store deletion. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[HE-4](hiring-workflow-e2e.md)** after Eligibility Composition Gate PASS. Feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`. Hiring E2E Acceptance Gate **not PASS**. RS-7 **not** executed. Eligibility Composition Gate **PASS**. Stage Authority Consumption Gate **PASS**. Hiring Acceptance Contract Gate **PASS**. min HR remains queued (feat locked; not scheduled). |
| **Queued Product successor** | **Hiring E2E program close** after Hiring E2E Acceptance Gate PASS — [brief](hiring-workflow-e2e.md). Do not start program close / execute RS-7 / schedule min HR in this amendment. min HR stays behind that close: startable only after close, still not auto-scheduled. Not leftover-store deletion. Not inherited DR1 / Mapping fixes. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM program = DONE** (Authority / Operator / 3A / 3B / Consumer Cutover `918274d1` + program close). **MA-1 Contract Gate = PASS**. **Mapping Resolution Gate = PASS**. **Mapping Operator Gate = PASS**. **Mapping Consumer Cutover Gate = PASS**. **Mapping program = DONE**. **Forms Publish Contract Gate = PASS**. **Publish Action Gate = PASS**. **Public Serve Gate = PASS**. **Operator Publish Gate = PASS**. **External Intake Acceptance Gate = PASS**. **External Intake program = DONE**. **Hiring Acceptance Contract Gate = PASS**. **Stage Authority Consumption Gate = PASS**. **Eligibility Composition Gate = PASS**. **Product = HE-4** (Acceptance walk feat open; Hiring E2E Acceptance Gate **not PASS**; RS-7 not executed; not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. Active Product is **HE-4** after Eligibility Composition Gate PASS. Feat `feat/hiring-e2e-he4-acceptance-walk` is open. Hiring E2E Acceptance Gate is **not PASS**. This amendment does not execute RS-7. Unlock ≠ silent schedule of Hiring E2E program close / min HR. Do not start leftover-store deletion. Do not convert FP-5 PASS into Release Acceptance PASS.  
...
| **P-FP2** | [FP-2](external-intake-forms-publish.md) Publish action runtime | **Publish Action Gate** ✅ — authenticated product route → `commit_publish`; leftover version bumps retired; republish idempotent per identity. Feat `feat/forms-publish-fp2-publish-action`. Not FP-3. Not leftover-store deletion. Not Hiring. Not P4 / P5 | Forms Publish Contract Gate | FP-3 |
| **P-FP3** | [FP-3](external-intake-forms-publish.md) Public serve from frozen publication | **Public Serve Gate** ✅ — public request → resolve live publication → frozen `form_publication_versions` snapshot → Form Runtime. Feat `feat/forms-publish-fp3-public-serve`. Not leftover-store deletion. Not Hiring. Not P4 / P5 | Publish Action Gate | FP-4 |
| **P-FP4** | [FP-4](external-intake-forms-publish.md) Operator publish surface | **Operator Publish Gate** ✅ — Never published → Publish (`commit_publish`) → Live v1 + public URL → Unpublish (`deactivate_endpoint`) → Inactive + URL absent. Feat `feat/forms-publish-fp4-operator-surface`. UI re-reads backend authority. Embed snippet is a later product slice. Not leftover-store deletion. Not Hiring. Not P4 / P5 | Public Serve Gate | FP-5 |
| **P-FP5** | [FP-5](external-intake-forms-publish.md) External submission | External Intake Acceptance Gate **PASS** — operator live public URL → stranger without auth opens → fills → submit → production intake → intake/person/application visible in workspace. Feat `feat/forms-publish-fp5-runtime`. Browser E2E through that URL. Not Mapping/RS-3. Not leftover-store deletion. Not Hiring. Not embed snippet. Not P4 / P5 | Operator Publish Gate | External Intake program close ✅ |
| **P-FP-X** | [External Intake](external-intake-forms-publish.md) program close | **External Intake program DONE** — outcome + release delta; authority / operator / E2E consumption PASS. Release Acceptance (RS-2) named, **not** Release Acceptance PASS. Evidence `4f454556` / [#386](https://github.com/igortatarynovich/HostFlow/pull/386) | External Intake Acceptance Gate | HE-1 (this amendment). min HR remains unlocked by Hiring close, **not** scheduled |
| **P-HE1** | [HE-1](hiring-workflow-e2e.md) Acceptance contract | **Hiring Acceptance Contract Gate** ✅ — the walk is defined step by step with the authoritative answerer per step; dual-evidence disposition sealed; “not a new Hiring Product” restated as a forbidden-implementation list; test-only seeding declared inadmissible as proof. SoT: [hiring-acceptance-contract.md](../architecture/hiring-acceptance-contract.md) (`hiring_acceptance.v1`). Named CI. Not HE-2 runtime. Not min HR. Not leftover-store deletion. Not RS-3 | External Intake program DONE | HE-2 |
| **P-HE2** | [HE-2](hiring-workflow-e2e.md) Stage authority consumption | **Stage Authority Consumption Gate** ✅ — hiring-path existence consumes LI-1; leftovers stop answering existence; transition-order rule `forward_moves_guarded_jumps_rejected` is production. SoT: [hiring-stage-authority-consumption.md](../architecture/hiring-stage-authority-consumption.md) (`hiring_stage_authority.v1`). Named CI. Not min HR. Not leftover-store deletion. Not RS-7 | Hiring Acceptance Contract Gate | HE-3 |
| **P-HE3** | [HE-3](hiring-workflow-e2e.md) Eligibility composition | **Eligibility Composition Gate** ✅ — one composed decision and one operator-readable refusal; requirement conjunct is RPM `r5_required_set`; v1/v2 are not eligibility authorities. SoT: [hiring-eligibility-composition.md](../architecture/hiring-eligibility-composition.md) (`hiring_eligibility_composition.v1`). Named CI. Not RS-7. Not min HR. Not a HE-2 stage-authority change | Stage Authority Consumption Gate | HE-4 |
| **P-HE4** | [HE-4](hiring-workflow-e2e.md) Acceptance walk | **Hiring E2E Acceptance Gate** — **not PASS**. Feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`. RS-7 not executed in this amendment. Close path = operator-configured tenant → product-surface stage moves → request/provide/accept document → Document Link + Candidate Evidence bind → readable refusal while an RPM requirement is unmet → requirement satisfied → successful transfer / `ready_for_employment.v1`. Inadmissible: `seed_documents_for_ready_for_handoff`, `candidate_evidence_helpers`, SQL/fixture inserts, `GET /transfer-readiness`. Not program close. Not min HR. Not inherited DR1 / Mapping fixes | Eligibility Composition Gate | Hiring E2E program close |

**Now:** Product **[HE-4](hiring-workflow-e2e.md)** after Eligibility Composition Gate PASS. Feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`. Hiring E2E Acceptance Gate **not PASS**. RS-7 **not** executed. Eligibility Composition Gate **PASS**. Stage Authority Consumption Gate **PASS**. Hiring Acceptance Contract Gate **PASS**. External Intake Acceptance Gate **PASS** (`4f454556`). Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. **Not** Mapping/RS-3. **Not** min HR (feat locked; not scheduled). **Not** Hiring E2E program close. **Not** inherited DR1 / Mapping fixes. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
E8-eval brief + feat **closed**. RPM program **DONE**. Mapping program **DONE**. Mapping Resolution Gate **PASS**. Mapping Operator Gate **PASS**. Mapping Consumer Cutover Gate **PASS**. Unlock does not schedule OCR / CL8 / Foundation / External Intake.
...
Product:
  CL0 → CL1 → LI-1 → DR1-contract → CL2 → CL3 → CL4 → CL5 → CL6 → CL7 → Vacancy Overlay Contract → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B → rpm-cutover → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1 → HE-2 → HE-3 → HE-4
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1 → HE-2 → HE-3 → HE-4   (this amendment)
```

Program horizon of this amendment: **HE-4** (Acceptance walk). Hiring E2E Acceptance Gate is **not PASS**. RS-7 is not executed. Eligibility Composition Gate is **PASS**. Stage Authority Consumption Gate is **PASS**. Hiring Acceptance Contract Gate is **PASS**. External Intake program close is **DONE**. Mapping program close is **DONE**. RPM program is **DONE**. min HR remains behind Hiring E2E program close, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

Hiring workflow E2E is **no longer in this table** — HE-4 is Active Product (feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**; RS-7 not executed). A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time. min HR stays feat locked until a later amendment after Hiring E2E program close.
...
|------|----------|--------|
| **Product critical path** | Forms Publish 5–7 · Hiring E2E 4–6 · min HR handoff 4–6 | **13–19** |
...
| **Serialized-equivalent total** | both tracks share one delivery capacity today | **29–42** |
...
**Moved −4–6 slices on 2026-09-20 after Mapping program close.** Mapping remaining slices leave the Product critical path. Forms Publish 5–7 remains. Serialized-equivalent **29–42**. Product **DONE** with no named successor until amendment. External Intake unlocked, **not** scheduled.

**Queue amendment 2026-09-21 names HE-1 Active Product.** External Intake program stays **DONE**. Serialized-equivalent stays **29–42** (no RC recut this amendment). Active Product → **HE-1** (brief; feat locked). min HR remains queued. Not leftover-store deletion. Not HE-1 contract seal this stamp.
...
**FP-4 Operator Publish Gate PASS 2026-09-20.** Never published → Publish → Live v1 + public URL → Unpublish → Inactive. Serialized-equivalent stays **29–42**. Active Product → **FP-5** (brief; feat locked that stamp).

**External Intake Acceptance Gate PASS 2026-09-21.** Feat `feat/forms-publish-fp5-runtime` from `de6383aa`. Serialized-equivalent stays **29–42**. Active Product → External Intake program close (brief; feat locked).

**FP-5 External submission feat opened 2026-09-21.** Branch `feat/forms-publish-fp5-external-submit` from `5e2a8f59`. Serialized-equivalent stays **29–42**. Active Product → **FP-5** (feat open; External Intake Acceptance Gate **not PASS**).

**FP-2 Publish Action feat opened 2026-09-20.** Branch `feat/forms-publish-fp2-publish-action` from `7112279e`. Serialized-equivalent stays **29–42**. Active Product → **FP-2** (feat open; Publish Action Gate **not PASS**).

**Publish Action Gate PASS 2026-09-20.** Authenticated `commit_publish` product route writes `form_publication_versions`; leftover version bumps retired; republish idempotent per identity. Serialized-equivalent stays **29–42**. Active Product → **FP-3** (brief; feat locked). Do not start FP-3 in this PR.

**FP-3 Public Serve feat opened 2026-09-20.** Branch `feat/forms-publish-fp3-public-serve` from `41be635a`. Serialized-equivalent stays **29–42**. Active Product → **FP-3** (feat open; Public Serve Gate **PASS**).
...
| **MA-1…MA-4 / Mapping program close** | `map-authority` / `map-resolve` / `map-operator` / `map-cutover` / `map-close` | [Mapping Authority](mapping-authority.md) internal slices; not a fourth mapping editor |
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: HE-4; feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**; RS-7 not executed).  
...
- **Mapping Authority** ← **DONE** — MA-1…MA-4 PASS · Consumer Cutover Gate **PASS** `fddadd39` · program close recorded — [brief](mapping-authority.md). Forms Publish Contract Gate **PASS**. Publish Action Gate **PASS**. Public Serve Gate **PASS**. Operator Publish Gate **PASS**. External Intake Acceptance Gate **PASS**. External Intake program **DONE**. Active Product = **[HE-4](hiring-workflow-e2e.md)** (feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**). min HR remains queued (feat locked; not scheduled).
...
| **46f** | **FP-2** Publish action runtime | [brief](external-intake-forms-publish.md) | after Forms Publish Contract Gate; **PASS** this PR |
| **46g** | **FP-3** Public serve from frozen publication | [brief](external-intake-forms-publish.md) | after Publish Action Gate; **PASS** this PR |
| **46h** | **FP-4** Operator publish surface | [brief](external-intake-forms-publish.md) | after Public Serve Gate; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface` |
| **46i** | **FP-5** External submission | [brief](external-intake-forms-publish.md) | after Operator Publish Gate; External Intake Acceptance Gate **PASS**; feat `feat/forms-publish-fp5-runtime` |
| **46j** | **External Intake program close** | [brief](external-intake-forms-publish.md) | **DONE** — outcome + release delta; not Release Acceptance PASS |
| **46k** | **HE-1** Acceptance contract | [brief](hiring-workflow-e2e.md) · [contract SoT](../architecture/hiring-acceptance-contract.md) | after External Intake program close; Hiring Acceptance Contract Gate **PASS** |
| **46l** | **HE-2** Stage authority consumption | [brief](hiring-workflow-e2e.md) · [consumption SoT](../architecture/hiring-stage-authority-consumption.md) | after Hiring Acceptance Contract Gate; Stage Authority Consumption Gate **PASS** |
| **46m** | **HE-3** Eligibility composition | [brief](hiring-workflow-e2e.md) · [composition SoT](../architecture/hiring-eligibility-composition.md) | after Stage Authority Consumption Gate; Eligibility Composition Gate **PASS** |
| **46n** | **HE-4** Acceptance walk | [brief](hiring-workflow-e2e.md) | after Eligibility Composition Gate; feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`; Hiring E2E Acceptance Gate **not PASS**; RS-7 not executed |
...
**Active (Product):** **[HE-4](hiring-workflow-e2e.md)** after Eligibility Composition Gate PASS. Feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`. Hiring E2E Acceptance Gate **not PASS**. RS-7 **not** executed. Eligibility Composition Gate **PASS**. Stage Authority Consumption Gate **PASS**. Hiring Acceptance Contract Gate **PASS**. External Intake Acceptance Gate **PASS** `4f454556`. Mapping Consumer Cutover Gate **PASS** `fddadd39`; evidence `92206f40`. E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6`. **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. **Not** Mapping/RS-3. **Not** min HR (feat locked; not scheduled). **Not** Hiring E2E program close. **Not** inherited DR1 / Mapping fixes. Do not invent CL8. Do not mark Foundation ✅. E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321). DR1-runtime ✅ [#313](https://github.com/igortatarynovich/HostFlow/pull/313). Overlay ✅ [#311](https://github.com/igortatarynovich/HostFlow/pull/311). E7 ✅ [#287](https://github.com/igortatarynovich/HostFlow/pull/287). Foundation stays 🔄.  
...
**Do not** park later Product on Engineering DONE. Product is **[HE-4](hiring-workflow-e2e.md)** after Eligibility Composition Gate PASS. Hiring E2E Acceptance Gate **not PASS**. Hiring Acceptance Contract Gate **PASS**.  
**Do not** auto-start min HR because Hiring is Active. Unlock ≠ schedule. Do not execute RS-7 in this amendment. Do not start Hiring E2E program close in this stamp. Do not start leftover-store deletion in this stamp. Do not mix Mapping / RS-3 / inherited DR1 fixes in this stamp. Do not convert FP-5 PASS into Release Acceptance PASS.
...
- 2026-09-20: **FP-3 Public Serve feat opened.** Branch `feat/forms-publish-fp3-public-serve` from `41be635a` ([#380](https://github.com/igortatarynovich/HostFlow/pull/380)). Close path = public request → resolve live publication → frozen `form_publication_versions` snapshot → canonical Form Runtime render. Leftover `/public/intake` → `form_presentation_runtime_v1` follows named expiry, not general cleanup. Embed is serve-surface compatibility, not FP-4 snippet UX. Public Serve Gate **not PASS**. An existing Adapter `resolve` / Form Runtime is not Gate PASS. This stamp does not ship runtime. Not Hiring / leftover-store deletion. Architecture stays CLOSED / PASS. Not CL8. Foundation stays 🔄. HostFlow v1 is not release-ready.
- 2026-09-20: **Publish Action Gate PASS.** Authenticated product route `POST /api/v1/platform/forms/{form_id}/publish` → Adapter `commit_publish` → `form_publication_versions`. Leftover presentation / Form Definition version bumps retired. Republish idempotent per identity. Active Product → **[FP-3](external-intake-forms-publish.md)** (brief; feat locked). Do not start FP-3 in this PR. Not Hiring / leftover-store deletion. Not P4 / P5. Architecture stays CLOSED / PASS. Not CL8. Foundation stays 🔄. HostFlow v1 is not release-ready.
- 2026-09-20: **FP-2 Publish Action feat opened.** Branch `feat/forms-publish-fp2-publish-action` from `7112279e` ([#378](https://github.com/igortatarynovich/HostFlow/pull/378)). Close path = authenticated product route → Adapter `commit_publish` → `form_publication_versions`; leftover presentation / Form Definition version bumps retire; republish is idempotent per identity. Publish Action Gate **not PASS**. An existing Adapter op is not Gate PASS. This stamp does not ship runtime. Not FP-3 / Hiring / leftover-store deletion. Architecture stays CLOSED / PASS. Not CL8. Foundation stays 🔄. HostFlow v1 is not release-ready.
