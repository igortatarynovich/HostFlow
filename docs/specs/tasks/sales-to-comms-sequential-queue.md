...
|-------|-------------|------|
| **Product** | **[HE-2](hiring-workflow-e2e.md)** after Hiring Acceptance Contract Gate PASS. Stage Authority Consumption Gate **PASS**. SoT: [hiring-stage-authority-consumption.md](../architecture/hiring-stage-authority-consumption.md) (`hiring_stage_authority.v1`). Do not start HE-3 / min HR / RS-7 in this PR. Not Mapping/RS-3. Not leftover-store deletion. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[HE-2](hiring-workflow-e2e.md)** after Hiring Acceptance Contract Gate PASS. Stage Authority Consumption Gate **PASS**. SoT: [hiring-stage-authority-consumption.md](../architecture/hiring-stage-authority-consumption.md) (`hiring_stage_authority.v1`). Hiring Acceptance Contract Gate **PASS**. min HR remains queued. |
| **Queued Product successor** | **HE-3** Eligibility answer collapse after Stage Authority Consumption Gate PASS — [brief](hiring-workflow-e2e.md). Do not start HE-3 / min HR / leftover-store deletion / RS-3 / RS-7 in this consumption PR. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. Active Product is **HE-2** after Hiring Acceptance Contract Gate PASS. Stage Authority Consumption Gate **PASS**. Unlock ≠ silent schedule of HE-3 / min HR. Do not start HE-3 runtime. Do not start leftover-store deletion. Do not convert FP-5 PASS into Release Acceptance PASS.  
...
| **P-HE2** | [HE-2](hiring-workflow-e2e.md) Stage authority consumption | **Stage Authority Consumption Gate** ✅ — hiring-path existence consumes LI-1; leftovers stop answering existence; transition-order rule `forward_moves_guarded_jumps_rejected` is production. SoT: [hiring-stage-authority-consumption.md](../architecture/hiring-stage-authority-consumption.md) (`hiring_stage_authority.v1`). Named CI. Not HE-3. Not min HR. Not leftover-store deletion. Not RS-7 | Hiring Acceptance Contract Gate | HE-3 |

**Now:** Product **[HE-2](hiring-workflow-e2e.md)** after Hiring Acceptance Contract Gate PASS. Stage Authority Consumption Gate **PASS**. SoT: [hiring-stage-authority-consumption.md](../architecture/hiring-stage-authority-consumption.md) (`hiring_stage_authority.v1`). Hiring Acceptance Contract Gate **PASS**. External Intake Acceptance Gate **PASS** (`4f454556`). Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. **Not** Mapping/RS-3. **Not** min HR. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
Product:
  CL0 → CL1 → LI-1 → DR1-contract → CL2 → CL3 → CL4 → CL5 → CL6 → CL7 → Vacancy Overlay Contract → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B → rpm-cutover → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1 → HE-2
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5 → External Intake program close → HE-1 → HE-2   (this amendment)
```

Program horizon of this amendment: **HE-2** (Stage Authority Consumption). Hiring Acceptance Contract Gate is **PASS**. External Intake program close is **DONE**. Mapping program close is **DONE**. RPM program is **DONE**. min HR remains unlocked by Hiring close, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

Hiring workflow E2E is **no longer in this table** — HE-2 is Active Product (Stage Authority Consumption Gate **PASS**; HE-3 feat locked). A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: HE-1; Hiring Acceptance Contract Gate **PASS**; HE-2 feat locked).  
...
| **46l** | **HE-2** Stage authority consumption | [brief](hiring-workflow-e2e.md) · [consumption SoT](../architecture/hiring-stage-authority-consumption.md) | after Hiring Acceptance Contract Gate; Stage Authority Consumption Gate **PASS**; HE-3 feat locked |
...
**Do not** park later Product on Engineering DONE. Product is **[HE-1](hiring-workflow-e2e.md)** after External Intake program close. Hiring Acceptance Contract Gate **PASS**.  
**Do not** auto-start min HR because Hiring is Active. Unlock ≠ schedule. Do not start HE-2 / RS-7 in this PR. Do not start leftover-store deletion in this stamp. Do not mix Mapping / RS-3 / embed snippet in this stamp. Do not convert FP-5 PASS into Release Acceptance PASS.
...
**Do** amend this queue when switching Product Active (this revision: Active Product = **HE-2**; Stage Authority Consumption Gate **PASS**; HE-3 feat locked).
