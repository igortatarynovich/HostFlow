...
|-------|-------------|------|
| **Product** | **[FP-5](external-intake-forms-publish.md)** after Operator Publish Gate **PASS**. Feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**. This stamp does not ship runtime. Not Mapping/RS-3. Not leftover-store deletion. Not Hiring E2E. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[FP-5](external-intake-forms-publish.md)** after Operator Publish Gate **PASS**. Feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**. This stamp does not ship runtime. Hiring / min HR remain queued. |
| **Queued Product successor** | External Intake program close after External Intake Acceptance Gate PASS — [brief](external-intake-forms-publish.md). Do not start leftover-store deletion / Hiring / RS-3 / embed snippet / a new form architecture in this stamp. Hiring is unlocked by RPM close, **not** scheduled. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM program = DONE** (Authority / Operator / 3A / 3B / Consumer Cutover `918274d1` + program close). **MA-1 Contract Gate = PASS**. **Mapping Resolution Gate = PASS**. **Mapping Operator Gate = PASS**. **Mapping Consumer Cutover Gate = PASS**. **Mapping program = DONE**. **Forms Publish Contract Gate = PASS**. **Publish Action Gate = PASS**. **Public Serve Gate = PASS**. **Operator Publish Gate = PASS**. **Product = FP-5** (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**; not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. This stamp opens feat `feat/forms-publish-fp5-external-submit`. Unlock ≠ silent schedule of Hiring. This stamp does not ship runtime. Do not start leftover-store deletion in this stamp.  
...
| **P-FP5** | [FP-5](external-intake-forms-publish.md) External submission | **External Intake Acceptance Gate** **not PASS** — live public URL → stranger without auth opens → fills → submit → production intake accepts → real intake/person/application per existing contracts. Feat `feat/forms-publish-fp5-external-submit` open. Later runtime proof is browser E2E through that URL, not API composition. This stamp does not ship runtime. Not Mapping/RS-3. Not leftover-store deletion. Not Hiring. Not embed snippet. Not P4 / P5 | Operator Publish Gate | External Intake program close |

**Now:** Product **[FP-5](external-intake-forms-publish.md)** (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**) after Operator Publish Gate **PASS**. Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. This stamp does not ship runtime. **Not** Mapping/RS-3. **Not** Hiring E2E. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5   (External Intake Acceptance Gate not PASS this stamp)
```

Program horizon of this amendment: **FP-5** (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`; Public Serve Gate **PASS**; FP-3 feat `feat/forms-publish-fp3-public-serve`). Publish Action Gate is **PASS**. Forms Publish Contract Gate is **PASS**. Mapping program close is **DONE**. RPM program is **DONE**. Hiring E2E is unlocked, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

External Intake / Forms Publish is **no longer in this table** — FP-5 is Active Product (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**) after Operator Publish Gate PASS. A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time.
...
**FP-5 External submission feat opened 2026-09-21.** Branch `feat/forms-publish-fp5-external-submit` from `5e2a8f59`. Serialized-equivalent stays **29–42**. Active Product → **FP-5** (feat open; External Intake Acceptance Gate **not PASS**). This stamp does not ship runtime.

**FP-2 Publish Action feat opened 2026-09-20.** Branch `feat/forms-publish-fp2-publish-action` from `7112279e`. Serialized-equivalent stays **29–42**. Active Product → **FP-2** (feat open; Publish Action Gate **not PASS**). This stamp does not ship runtime.
...
**FP-3 Public Serve feat opened 2026-09-20.** Branch `feat/forms-publish-fp3-public-serve` from `41be635a`. Serialized-equivalent stays **29–42**. Active Product → **FP-3** (feat open; Public Serve Gate **PASS**). This stamp does not ship runtime.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: **FP-5**; feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**; Operator Publish Gate **PASS**; Public Serve Gate **PASS**).  
...
| **46i** | **FP-5** External submission | [brief](external-intake-forms-publish.md) | after Operator Publish Gate; feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS** |
...
**Active (Product):** **[FP-5](external-intake-forms-publish.md)** (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**) after Operator Publish Gate **PASS**. Mapping Consumer Cutover Gate **PASS** `fddadd39`; evidence `92206f40`. E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6`. **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. This stamp does not ship runtime. **Not** Mapping/RS-3. **Not** Hiring E2E. Do not invent CL8. Do not mark Foundation ✅. E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321). DR1-runtime ✅ [#313](https://github.com/igortatarynovich/HostFlow/pull/313). Overlay ✅ [#311](https://github.com/igortatarynovich/HostFlow/pull/311). E7 ✅ [#287](https://github.com/igortatarynovich/HostFlow/pull/287). Foundation stays 🔄.  
...
**Do not** park later Product on Engineering DONE. Product is FP-5 after Operator Publish Gate PASS (feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**; feat `feat/forms-publish-fp4-operator-surface`; FP-3 feat `feat/forms-publish-fp3-public-serve`).  
**Do not** auto-start Hiring E2E / min HR because FP-5 is named. Hiring is unlocked by RPM close — unlock ≠ schedule. Do not start leftover-store deletion in this stamp. This stamp does not ship runtime. Do not mix Mapping / RS-3 / embed snippet in this stamp.
...
**Do** amend this queue when switching Product Active (this revision: **FP-5**; feat `feat/forms-publish-fp5-external-submit` open; External Intake Acceptance Gate **not PASS**; Operator Publish Gate **PASS**; Public Serve Gate **PASS**).
