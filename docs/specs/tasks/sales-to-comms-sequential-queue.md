...
|-------|-------------|------|
| **Product** | External Intake program close ([brief](external-intake-forms-publish.md); feat locked) after External Intake Acceptance Gate **PASS**. Feat `feat/forms-publish-fp5-runtime`. Not Mapping/RS-3. Not leftover-store deletion. Not Hiring E2E. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | External Intake program close ([brief](external-intake-forms-publish.md); feat locked) after External Intake Acceptance Gate **PASS**. Feat `feat/forms-publish-fp5-runtime`. Hiring / min HR remain queued. |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. This stamp ships feat `feat/forms-publish-fp5-runtime` and stamps External Intake Acceptance Gate **PASS**. Unlock ≠ silent schedule of Hiring. Do not start leftover-store deletion in this stamp.  
...
**Now:** Product External Intake program close ([brief](external-intake-forms-publish.md); feat locked) after External Intake Acceptance Gate **PASS**. Feat `feat/forms-publish-fp5-runtime`. Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. **Not** Mapping/RS-3. **Not** Hiring E2E. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4 → FP-5   (External Intake Acceptance Gate **PASS** this stamp)
```

Program horizon of this amendment: External Intake program close (feat `feat/forms-publish-fp5-runtime`; External Intake Acceptance Gate **PASS**; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`; Public Serve Gate **PASS**; FP-3 feat `feat/forms-publish-fp3-public-serve`). Publish Action Gate is **PASS**. Forms Publish Contract Gate is **PASS**. Mapping program close is **DONE**. RPM program is **DONE**. Hiring E2E is unlocked, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

External Intake / Forms Publish is **no longer in this table** — Active Product is External Intake program close (feat `feat/forms-publish-fp5-runtime`; External Intake Acceptance Gate **PASS**) after FP-5. A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: External Intake program close; feat `feat/forms-publish-fp5-runtime`; External Intake Acceptance Gate **PASS**; Operator Publish Gate **PASS**; Public Serve Gate **PASS**).  
...
- **Mapping Authority** ← **DONE** — MA-1…MA-4 PASS · Consumer Cutover Gate **PASS** `fddadd39` · program close recorded — [brief](mapping-authority.md). Forms Publish Contract Gate **PASS**. Publish Action Gate **PASS**. Public Serve Gate **PASS**. Operator Publish Gate **PASS**. Active Product = External Intake program close ([brief](external-intake-forms-publish.md); feat locked) after External Intake Acceptance Gate **PASS**. Feat `feat/forms-publish-fp5-runtime`. Hiring / min HR remain queued.
...
**Do not** park later Product on Engineering DONE. Product is External Intake program close after External Intake Acceptance Gate **PASS** (feat `feat/forms-publish-fp5-runtime`; feat `feat/forms-publish-fp4-operator-surface`; FP-3 feat `feat/forms-publish-fp3-public-serve`).  
**Do not** auto-start Hiring E2E / min HR because External Intake Acceptance Gate is PASS. Hiring is unlocked by RPM close — unlock ≠ schedule. Do not start leftover-store deletion in this stamp. Do not mix Mapping / RS-3 / embed snippet in this stamp.
...
**Do** amend this queue when switching Product Active (this revision: External Intake program close; feat `feat/forms-publish-fp5-runtime`; External Intake Acceptance Gate **PASS**; Operator Publish Gate **PASS**; Public Serve Gate **PASS**).
