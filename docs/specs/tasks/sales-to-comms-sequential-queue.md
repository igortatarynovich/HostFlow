...
|-------|-------------|------|
| **Product** | **Entity Workspace D9** Services Order Cutover — [brief](entity-workspace-d9-services-order-cutover.md); feat; named Cutover Gate; `HrHandoffDetailPage` / Documents Phase E out | Almost all capacity |
...
- **Entity Workspace D9 — Services Order Cutover** ← **active** (feat; named Cutover Gate) — [brief](entity-workspace-d9-services-order-cutover.md); `HrHandoffDetailPage` out  
...
| **27** | **Entity Workspace D9** Services `/app/orders` | `docs/…` ✅ [#267](https://github.com/igortatarynovich/HostFlow/pull/267) · `feat/entity-workspace-d9-services-order-cutover` | **active** (feat; named Cutover Gate) — [brief](entity-workspace-d9-services-order-cutover.md) |

**C0–C2.3** ✅. **C2.4 frozen.** **Epic C — complete.** **A2 — PASS_WITH_CONSTRAINTS.** Forms Foundation ✅. D1 ✅. D2 ✅. D3 ✅. D4 ✅. D5 ✅. D6 ✅. D7 ✅. D8 ✅.  
**Active (Product):** Entity Workspace D9 — [Services Order Cutover](entity-workspace-d9-services-order-cutover.md) (feat; named Cutover Gate). Next = Documents Phase E (locked).  
...
### Entity Workspace D9 — Services order cutover ← **active** (feat; named Cutover Gate)

[entity-workspace-d9-services-order-cutover.md](entity-workspace-d9-services-order-cutover.md). Services order (`ServicesPage` / `/app/orders` · `service_order`) binds to D2 enabled slots. Named D9 Cutover Gate. `HrHandoffDetailPage` / Documents Phase E locked.
...
**Do not** treat Shell/chrome as Phase D Universal done; **do not** enable D2 Documents slot before Phase E; **do not** start Documents Phase E or Forms P3–P5 before D9 feat merges; **do not** treat Shell `documents` nav, Vacancy docs section, HR dossier, or Services billing tab as D2 `documents` enable; **do not** cut over `HrHandoffDetailPage` or re-bind Sales Order in D9.  
**Do** amend this queue when switching Product Active (this revision: live = Entity Workspace D9 feat; named Cutover Gate).
