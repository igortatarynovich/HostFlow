...
| **Active Launch-ops** | **[OL-1](operate-and-launch.md) Launch contract & ownership seal** (docs only) — opened by the 2026-08-31 amendment below. RR3 / RR4 / RR7 owner: **igortatarynovich** for all three, which OL-1 records as a named residual rather than as a solved question. Production target for v1: **one dedicated host, one compose stack** |
| **Queued Launch-ops** | **OL-2** Deploy, migrate & rollback, then OL-3…OL-7 — [brief](operate-and-launch.md). The migration blocker (`alembic upgrade heads` fails on a fresh DB, [AGENTS.md](../../../AGENTS.md)) is owned by **OL-2**, not by the pytest background item |
