...
|-------|-------------|------|
| **Product** | **[FP-5](external-intake-forms-publish.md)** after Operator Publish Gate **PASS**. Feat `feat/forms-publish-fp4-operator-surface`. Do not start FP-5 / embed in this PR. Not leftover-store deletion. Not Hiring E2E. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[FP-5](external-intake-forms-publish.md)** after Operator Publish Gate **PASS**. Feat `feat/forms-publish-fp4-operator-surface`. Do not start FP-5 / embed in this PR. Hiring / min HR remain queued. |
| **Queued Product successor** | none until amendment — FP-5 is named Active Product (brief; feat locked; not started). Do not start leftover-store deletion / Hiring / RS-3 / embed snippet in this stamp. Hiring is unlocked by RPM close, **not** scheduled. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM program = DONE** (Authority / Operator / 3A / 3B / Consumer Cutover `918274d1` + program close). **MA-1 Contract Gate = PASS**. **Mapping Resolution Gate = PASS**. **Mapping Operator Gate = PASS**. **Mapping Consumer Cutover Gate = PASS**. **Mapping program = DONE**. **Forms Publish Contract Gate = PASS**. **Publish Action Gate = PASS**. **Public Serve Gate = PASS**. **Product = FP-5** (Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`; not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. This stamp closes Operator Publish Gate on feat `feat/forms-publish-fp4-operator-surface`. Unlock ≠ silent schedule of Hiring. Do not start FP-5 / embed in this PR. Do not start leftover-store deletion in this stamp.  
...
**Now:** Product **[FP-5](external-intake-forms-publish.md)** (brief; feat locked; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`) after Public Serve Gate **PASS**. Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. Do not start FP-5 / embed in this PR. **Not** Hiring E2E. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
**FP-2 Publish Action feat opened 2026-09-20.** Branch `feat/forms-publish-fp2-publish-action` from `7112279e`. Serialized-equivalent stays **29–42**. Active Product → **FP-2** (feat open; Publish Action Gate **not PASS**). Do not start FP-5 / embed in this PR.
...
**FP-3 Public Serve feat opened 2026-09-20.** Branch `feat/forms-publish-fp3-public-serve` from `41be635a`. Serialized-equivalent stays **29–42**. Active Product → **FP-3** (feat open; Public Serve Gate **PASS**). Do not start FP-5 / embed in this PR.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: **FP-5**; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`; Public Serve Gate **PASS**).  
...
**Do not** park later Product on Engineering DONE. Product is FP-5 after Operator Publish Gate PASS (feat `feat/forms-publish-fp4-operator-surface`; FP-3 feat `feat/forms-publish-fp3-public-serve`).  
**Do not** auto-start Hiring E2E / min HR because FP-3 is named. Hiring is unlocked by RPM close — unlock ≠ schedule. Do not start leftover-store deletion in this stamp. Do not start FP-5 / embed in this PR. Do not start FP-5 / embed snippet in this stamp.
...
**Do** amend this queue when switching Product Active (this revision: **FP-5**; Operator Publish Gate **PASS**; feat `feat/forms-publish-fp4-operator-surface`; Public Serve Gate **PASS**).
