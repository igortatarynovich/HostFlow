...
|-------|-------------|------|
| **Product** | **[FP-4](external-intake-forms-publish.md)** after Public Serve Gate **PASS**. Feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**. This stamp does not ship operator UI. Not leftover-store deletion. Not Hiring E2E. Not OCR. Do not invent CL8. Do not mark Foundation ✅ | Almost all capacity |
...
| Role | Value |
|------|--------|
| **Active Product** | **[FP-4](external-intake-forms-publish.md)** after Public Serve Gate **PASS**. Feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**. This stamp does not ship operator UI. Hiring / min HR remain queued. |
| **Queued Product successor** | **FP-5** Acceptance bind after Operator Publish Gate PASS — [brief](external-intake-forms-publish.md). Do not start leftover-store deletion / Hiring / RS-3 / embed snippet in this stamp. Hiring is unlocked by RPM close, **not** scheduled. Not OCR / packages / automation plane / extensions / Billing product / AI. Do not invent CL8. Do not mark Foundation ✅. |
...
| **Phase E** | **E7 = DONE**. **E8-bind = DONE** (Gate PASS [#321] / `8246421f`). **E8-eval = DONE** (Gate PASS [#324] / `19c95ef6`). **RPM program = DONE** (Authority / Operator / 3A / 3B / Consumer Cutover `918274d1` + program close). **MA-1 Contract Gate = PASS**. **Mapping Resolution Gate = PASS**. **Mapping Operator Gate = PASS**. **Mapping Consumer Cutover Gate = PASS**. **Mapping program = DONE**. **Forms Publish Contract Gate = PASS**. **Publish Action Gate = PASS**. **Public Serve Gate = PASS**. **Product = FP-4** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**; not a Phase E leftover) |
...
1. **One Active Product slice**, **or Product DONE with no named successor until amendment**. This stamp opens feat `feat/forms-publish-fp4-operator-surface`. Unlock ≠ silent schedule of Hiring. This stamp does not ship operator UI. Do not start leftover-store deletion in this stamp.  
...
| **P-FP4** | [FP-4](external-intake-forms-publish.md) Operator publish surface | **Operator Publish Gate** **not PASS** — operator opens form → publication state/version → publish/unpublish → resulting live state → public URL. Feat `feat/forms-publish-fp4-operator-surface` open. This stamp does not ship operator UI. Embed snippet is a later product slice. Not leftover-store deletion. Not Hiring. Not P4 / P5 | Public Serve Gate | FP-5 |

**Now:** Product **[FP-4](external-intake-forms-publish.md)** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**) after Public Serve Gate **PASS**. Mapping Consumer Cutover Gate **PASS** (`fddadd39`; evidence `92206f40`). **v1 scope:** [Release Goal](../gates/hostflow-v1-release-goal.md). **Not** leftover-store deletion. This stamp does not ship operator UI. **Not** Hiring E2E. **Not** OCR. Do not invent CL8. Do not mark Foundation ✅.
...
Documents:
  ...
  E8-eval ∧ DAG review [#328] → RPM-1 → … → RPM program close → MA-1 → MA-2 → MA-3 → MA-4 → Mapping program close → FP-1 → FP-2 → FP-3 → FP-4   (Operator Publish Gate not PASS this stamp)
```

Program horizon of this amendment: **FP-4** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**; Public Serve Gate **PASS**; FP-3 feat `feat/forms-publish-fp3-public-serve`). Publish Action Gate is **PASS**. Forms Publish Contract Gate is **PASS**. Mapping program close is **DONE**. RPM program is **DONE**. Hiring E2E is unlocked, **not** scheduled. Do not invent CL8. Do not auto-start OCR. Do not mark Foundation ✅.

### v1 blocker programs beyond this horizon (briefed, not scheduled)

External Intake / Forms Publish is **no longer in this table** — FP-4 is Active Product (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**) after Public Serve Gate PASS. A brief is **not** a schedule for the rows below: they exist so the release distance is countable. Activation still requires a queue amendment, one Active Product slice at a time.
...
**FP-2 Publish Action feat opened 2026-09-20.** Branch `feat/forms-publish-fp2-publish-action` from `7112279e`. Serialized-equivalent stays **29–42**. Active Product → **FP-2** (feat open; Publish Action Gate **not PASS**). This stamp does not ship operator UI.
...
**FP-3 Public Serve feat opened 2026-09-20.** Branch `feat/forms-publish-fp3-public-serve` from `41be635a`. Serialized-equivalent stays **29–42**. Active Product → **FP-3** (feat open; Public Serve Gate **PASS**). This stamp does not ship operator UI.
...
1. Exactly one **Active Product** slice, **or Product DONE with no named successor until amendment** (now: **FP-4**; feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**; Public Serve Gate **PASS**).  
...
- **Mapping Authority** ← **DONE** — MA-1…MA-4 PASS · Consumer Cutover Gate **PASS** `fddadd39` · program close recorded — [brief](mapping-authority.md). Forms Publish Contract Gate **PASS**. Publish Action Gate **PASS**. Public Serve Gate **PASS**. Active Product = **[FP-4](external-intake-forms-publish.md)** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**). Hiring / min HR remain queued.
...
| **46h** | **FP-4** Operator publish surface | [brief](external-intake-forms-publish.md) | after Public Serve Gate; feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS** |
...
**Do not** park later Product on Engineering DONE. Product is FP-4 after Public Serve Gate PASS (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**; FP-3 feat `feat/forms-publish-fp3-public-serve`).  
**Do not** auto-start Hiring E2E / min HR because FP-3 is named. Hiring is unlocked by RPM close — unlock ≠ schedule. Do not start leftover-store deletion in this stamp. This stamp does not ship operator UI. Do not start FP-5 / embed snippet in this stamp.
...
**Do** amend this queue when switching Product Active (this revision: **FP-4**; feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**; Public Serve Gate **PASS**).
