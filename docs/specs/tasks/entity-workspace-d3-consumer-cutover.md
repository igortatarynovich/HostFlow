...
**Branch (docs):** `docs/entity-workspace-d3-consumer-cutover`  
**Branch (code):** `feat/entity-workspace-d3-consumer-cutover` (locked)  
...
|---------|------------|-----------|
| Sales Inquiry detail (`SalesInquiryDetailPanel` + questionnaire / timeline / rodo / duplicates sections) | Module-local panel stack | **First consumer** — must bind to D2 slots |
...
| **D3** | First consumer cutover (Sales Inquiry) | **this brief** (feat locked) |
| **D4+** | Remaining consumers / Shell section collapse | locked until D4 brief |

---

## Entity Workspace D3 Cutover Gate (CI — mandatory after feat)
...
- [ ] Brief merged  
- [ ] Named Entity Workspace D3 Cutover Gate (feat)  
- [ ] First consumer = Sales Inquiry; Candidate/HR not cut over  
- [ ] Reserved `documents` cannot be enabled  
- [ ] Shell `EntityWorkspaceSectionId` not collapsed into D2 slots  
- [ ] No Catalog Passport mint  
- [ ] D1+D2 gates still green; Entity Foundation remains 🔄  
- [ ] Forms P3–P5 and Documents Phase E stay out of Product Track  
- Product Track stays D3 until D4 brief opens

---

## DoD

- [ ] Brief sealed with first consumer + in/out + acceptance  
- [ ] Queue + roadmap + AGENTS point at this brief  
- [ ] D2 marked complete with #254 merge ref  
