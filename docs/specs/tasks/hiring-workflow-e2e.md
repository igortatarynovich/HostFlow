...
**Status:** **QUEUED** (brief only; feat locked; **not scheduled**) — Active Product is External Intake program close ([brief](external-intake-forms-publish.md); feat locked)
...
> Opening this brief does **not** schedule it. RPM program close **unlocks** Hiring (policy-authority edge). Unlock ≠ schedule. MA-4 Cutover Gate PASS. Leftover-store deletion is not this program. The queue’s Active Product is External Intake program close ([brief](external-intake-forms-publish.md); feat locked).
>
