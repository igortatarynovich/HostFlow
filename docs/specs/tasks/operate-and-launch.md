...
**OL-2C (2026-08-31) — [CI / RB-2 proof-path](operate-launch-ol2c-ci-parity.md).** One path: `postgres:16-alpine`, repo-root `alembic.ini`, fresh DB → migrate → start → `/healthz`. `/build` and admin login become mandatory when #332 and #336 are on the tree. This is the last technical slice before the first trusted-base baseline release.
...
- 2026-08-31 (later, OL-2C): CI aligned to `postgres:16-alpine`; canonical Alembic is repo-root `alembic.ini`; proof-path starts the app. [operate-launch-ol2c-ci-parity.md](operate-launch-ol2c-ci-parity.md).
- 2026-08-31 (later, OL-2B first-admin): **Bootstrap admin schema closed as implementation, not as gate execution.** `superadmin` is an ADR-036 persisted trust role; `users.preferences` is a required empty object. Fresh Postgres 16 rejected both until `202608310001_bootstrap_admin_schema`. The throwaway composite `11f1c845` was evidence only; the first baseline must be rebuilt from a later trusted-base HEAD. OL-2B remains **PASS_IMPLEMENTATION / NOT EXECUTED**.
- 2026-08-31 (later, OL-2A executable): release-path artefact identity landed — OCI-labelled backend image, `GET /build`, frontend `build.json` + content-hash atomic publish, throwaway `deploy/compose.release.yml`. **Live stack unchanged.** No tag created.
