...
- 2026-08-31 (later, OL-2B first pass): throwaway attempt on the production host using the OL-2A release path. Artefacts built (`7cb67703`, image digest recorded then pruned). Frontend `build.json` missed `--outDir` (fixed on OL-2A as `40fa8160`). `compose.release.yml` STOP — host disk 100%, `initdb` could not create `pg_wal`; live backend restarted under disk pressure. Sidecar Postgres 16 on `:55432` migrated to head; first-admin still FAIL (`superadmin` / `preferences`). [RB-2](../../runbooks/rb-2-migrate-and-bootstrap.md) drafted from that path. Sequence not completed: backend-start / frontend-served / `/build` not reached. Not an execution record.
- 2026-08-31 (later, OL-2A executable): release-path artefact identity landed on `feat/operate-launch-ol2a-artefact-identity` (`cdfb8697`) — OCI-labelled backend image, `GET /build`, frontend `build.json` + content-hash atomic publish, throwaway `deploy/compose.release.yml` with no source bind mounts. **Live stack unchanged.** Contract C-1…C-3 hold on that path only; C-5 first predecessor and C-6 retained store are still open. No tag created.
