...
**OL-2C (2026-08-31) — [CI / RB-2 proof-path](operate-launch-ol2c-ci-parity.md).** One path: `postgres:16-alpine`, repo-root `alembic.ini`, fresh DB → migrate → bootstrap → start → `/healthz` 200 → `/build` 200 + SHA → admin login 200. Failure of `/build` or login is an OL-2C defect.
