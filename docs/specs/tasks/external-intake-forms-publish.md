...
**Status:** **ACTIVE** — Forms Publish Contract Gate **PASS**. Publish Action Gate **PASS**. Public Serve Gate **PASS**. Active Product = **FP-4** (brief; feat locked).
**Phase class:** platform
**Branch (docs):** `docs/forms-publish-fp1-contract-seal`
**Branch (code):** `feat/forms-publish-fp3-public-serve`. Public Serve Gate **PASS**. Do not start FP-4 in this stamp.
...
> Mapping program is **DONE**. Unlock ≠ schedule of leftover-store deletion / Hiring. The queue’s Active Product is **FP-4** (brief; feat locked) after Public Serve Gate **PASS**. Do not start FP-4 in this stamp.
...
The roadmap no longer forbids the work v1 cannot ship without, and the epic no longer contradicts the Release Goal. **FP-1 sealed the publish semantics** (`forms_publish.v1`). **FP-2 Publish Action Gate PASS**. Feat `feat/forms-publish-fp3-public-serve` is open. Public Serve Gate **PASS**. Do not start FP-4 in this stamp.
...
## FP-4 — Operator publish surface (Active Product; feat locked; Operator Publish Gate **not PASS**)

The operator job: publish, unpublish, see what is live, get the URL. Out: theming, A/B, analytics dashboards. Do not start FP-4 in this stamp.
...
**Active Product:** **FP-4** (brief; feat locked). Public Serve Gate **PASS**. Publish Action Gate **PASS**. Forms Publish Contract Gate **PASS**.  
**Unlocks:** nothing automatically — “unlock ≠ schedule”  
**Does not:** start FP-4 operator UI / snippet in this stamp; mint a second renderer, publication state, or submit engine; open P4 / P5; migrate `TenantLeadForm` → FormTemplate SoT (U-5 residual: Publish ships on the bridge, and **no new writer may be added to it**); rebuild Shared Intake; touch C2.4; start leftover-store deletion / Hiring; expand Mapping Operator Surface inherited reds

**Does:** ship public request → Adapter resolve live publication → frozen snapshot → Form Runtime on feat `feat/forms-publish-fp3-public-serve`. ADR-022 stays Accepted without expansion.
