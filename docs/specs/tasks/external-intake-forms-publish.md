...
**Status:** **ACTIVE** — Forms Publish Contract Gate **PASS**. Publish Action Gate **PASS**. Public Serve Gate **PASS**. Active Product = **FP-4** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**).
**Phase class:** platform
**Branch (docs):** `docs/forms-publish-fp1-contract-seal`
**Branch (code):** `feat/forms-publish-fp4-operator-surface`. Operator Publish Gate **not PASS**. This stamp does not ship operator UI.
...
> Mapping program is **DONE**. Unlock ≠ schedule of leftover-store deletion / Hiring. The queue’s Active Product is **FP-4** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**) after Public Serve Gate **PASS**. This stamp does not ship operator UI.
...
| **FP-4** | Operator publish surface | `fp-operator` | **Operator Publish Gate** **not PASS** — operator opens form → publication state/version → publish/unpublish → resulting live state → public URL from product UI. Feat `feat/forms-publish-fp4-operator-surface` open. This stamp does not ship operator UI | FP-3 Gate | 1 slice |
...
## FP-4 — Operator publish surface (Active; feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**)

Close path (operator): **operator opens form → sees current publication state/version → publish or unpublish → sees resulting live state → obtains public URL from product UI**. Publish / unpublish call closed FP-2 authority (`commit_publish` / lifecycle). Live state and public URL read closed FP-3 authority (live publication → frozen snapshot → Form Runtime `serve()`). No new publication or serve semantics. Embed snippet is a later product slice and keeps the one-serve-surface invariant. This stamp does not ship operator UI. Out: embed snippet; themes / analytics; a second serve or publish mechanism; leftover-store deletion; Hiring; P4 / P5; Mapping Operator Surface inherited red at `54537f00`.
...
**Active Product:** **FP-4** (feat `feat/forms-publish-fp4-operator-surface` open; Operator Publish Gate **not PASS**). Public Serve Gate **PASS**. Publish Action Gate **PASS**. Forms Publish Contract Gate **PASS**.  
**Unlocks:** nothing automatically — “unlock ≠ schedule”  
**Does not:** ship operator UI / production in this stamp; add embed snippet; mint a second renderer, publication state, or submit engine; open P4 / P5; migrate `TenantLeadForm` → FormTemplate SoT (U-5 residual: Publish ships on the bridge, and **no new writer may be added to it**); rebuild Shared Intake; touch C2.4; start leftover-store deletion / Hiring; expand Mapping Operator Surface inherited reds

**Does:** open feat `feat/forms-publish-fp4-operator-surface` against sealed [forms-publish-contract.md](../architecture/forms-publish-contract.md) (`forms_publish.v1`). Close path stays `operator opens form → publication state/version → publish/unpublish → resulting live state → public URL`. Publish uses FP-2. Live/public URL uses FP-3. ADR-022 stays Accepted without expansion.
