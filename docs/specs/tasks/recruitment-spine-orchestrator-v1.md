...
6. **Three-host Full Spine Gate** — [`three-host-full-spine-gate-pem1.md`](three-host-full-spine-gate-pem1.md) **NOT PASS** (kernel ≠ PEM-1; architectural hold).
7. **Platform next:** Accept [`ADR-042`](../architecture/ADR-042-spine-policy-separation.md) (life path → process → module → policy) → complete [`spine-policy-separation-inventory.md`](spine-policy-separation-inventory.md) leak overlay → baseline **Full Spine kernel** proof. Hiring E2E not opened.
