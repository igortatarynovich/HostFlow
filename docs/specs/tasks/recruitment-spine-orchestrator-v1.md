...
5. **Slice 4 PASS** — [`employment-start-allowed-eso5-enforcement.md`](employment-start-allowed-eso5-enforcement.md) @ `b8c9dd04` (Confirm requires `start_allowed=true`; mint-on-confirm retired). **STOP.** Three-host Full Spine Gate = separate decision (not auto-opened); Hiring E2E program not opened.
