...
- **Documents:** используются для фильтрации кандидатов, подходящих для вакансии (по типу разрешения на работу, визе и т.д.). Vacancy-level document setup допустим только как `template selector + vacancy-specific override`, а не как второй независимый редактор document requirements.  
