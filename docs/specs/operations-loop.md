...
**Параллельная подсистема — Planner:**

```
   CommunicationsCalendarPage  ──┐
   (drag-and-drop UI)            │
                                 ▼
   communication_planner_events table ─── только для календаря
   (start_at, end_at, kind, status, assignee, entity)
```

Planner ≠ reminder. Planner — это запланированный «слот в календаре» (звонок, встреча, follow-up). Reminder — это «надо что-то сделать к дедлайну». Сейчас они **раздельны**, и пользователь этой границы не видит. См. §6 «Гэп: дублирующиеся сущности».
...
- **Drag-to-reschedule:** `PATCH /communications/planner/events/{id}` — **только для planner-событий**, reminders двигать нельзя.

### 2.4 Communications planner

- **Модель:** `CommunicationPlannerEvent` (`models/communication.py` 179–209) — `kind` (task/call/meeting/followup/shift), `status` (planned/in_progress/done/cancelled), `start_at/end_at`, `assignee_id`, `entity_type/id`.
- **API:** `backend/app/api/v1/communications/routes/planner.py` (470 LOC) — `GET/POST/PATCH /communications/planner/events`.
- **Working hours:** проверяются ТОЛЬКО на фронте при batch-создании. Backend `POST /planner/events` принимает любой `start_at` (валидирует только `end_at >= start_at`).
...
|---|---|---|
| **Reminder** vs **Planner event** | Две разные таблицы, два API. | «Я создал событие в календаре — почему оно не в задачах?» |
...
- `/api/v1/communications/planner/events` тот же.
...
- **DONE — focus-by-id на `/app/calendar` (G-6 Stage 2f).** Query: `?event_id=<uuid>` (alias `eventId`). Семантика UUID: сначала как **planner event id** (`GET /api/v1/communications/planner/events/{id}` если строка ещё не в загруженном list-window), иначе как **activity/reminder id** (`GET /api/v1/activities/{id}`). Реализация: `hostflow-frontend/src/pages/CommunicationsCalendarPage.tsx` — merge fetch в `focusInjectPlanner` / `focusInjectReminder` → `plannerEventsEffective` / `remindersEffective` → `unifiedEvents`; при матче: `viewMode=day`, `selectedDay=dateKey`, курсор month/week на день, сброс фильтров источника на `all`, `scrollIntoView` на элемент `id=hf-cal-ev-<unified.id>` (day-bucket card), для planner — `activePlannerMenuId` (quick-actions), кольцо highlight ~4.5s, `replace` strip query. `TodayPlannerPanel` row-link: `/app/calendar?event_id=<planner-id>`. Backend: `backend/app/api/v1/communications/routes/planner.py::get_planner_event`, `backend/app/api/v1/activities_v1.py::get_activity` + `reminder_tasks.get_reminder_for_actor`. Тесты: `backend/tests/api/test_planner_event_get.py`, `backend/tests/api/test_activity_get.py` (404 smoke).
