...
**Product Track:** **[FP-4](../tasks/external-intake-forms-publish.md)** — Public Serve Gate **PASS**. Publish Action Gate **PASS**. Forms Publish Contract Gate **PASS** ([forms-publish-contract.md](../architecture/forms-publish-contract.md); `forms_publish.v1`). FP-4 feat locked; Operator Publish Gate **not PASS** — scheduled in the [sequential queue](../tasks/sales-to-comms-sequential-queue.md). This file does not invent slice order.
...
| **3** | **External Intake / Forms Publish** — [brief](../tasks/external-intake-forms-publish.md) (**active**; FP-1 Gate PASS; FP-2 Gate PASS; FP-3 Public Serve Gate **PASS**; FP-4 feat locked) | `publish → public form → submit → mapping → canonical entity → visible in workspace`. Forms P4 / P5 stay later. |
