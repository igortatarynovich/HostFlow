...
**Product Track:** **[MA-2](../tasks/mapping-authority.md)** (brief; feat locked) after Mapping Authority Contract Gate PASS — scheduled in the [sequential queue](../tasks/sales-to-comms-sequential-queue.md). This file does not invent slice order.
...
| **2** | **Mapping Authority** — [brief](../tasks/mapping-authority.md) (Active: MA-2; MA-1 Contract Gate PASS; brief; feat locked) | One operator-visible model from source answers to **canonical entity fields**. Not “build another mapping editor.” |
...
| Mapping Authority | **PASS** (sole write = intake_source_profiles.mapping_rules; leftovers classified; [mapping-authority-contract.md](../architecture/mapping-authority-contract.md)) | partial (C-5 / intake editors; leftover Meta admin UI) | partial | OPEN (runtime / Resolution Gate not this close) |
...
- Does not schedule External Intake, Hiring E2E, or min HR (MA-2 is scheduled in the sequential queue, not by this file; the Launch-ops track is opened by a queue amendment)  
- Does not open Mapping feat or mark Mapping Resolution Gate PASS (MA-1 Contract Gate is PASS; feat stays locked)  
