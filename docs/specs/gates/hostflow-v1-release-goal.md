...
**Commercial packaging & modularity (v1):** v1 proves the unified `Source → Recruitment → Handoff → Employment → Employability → Formalize → Employee → Started` Spine as **one commercial product** that a client can run end-to-end reliably. Individual Spine blocks do **not** need standalone commercial validation. **Full technical module independence** (perfect isolation, standalone runtime, every `enabled_modules` combo) is **not** a release requirement and may finish after launch. Only **release-critical** boundaries hold the launch (data integrity, critical-state ownership, Spine contracts, no dangerous cycles, safe evolution). [`ADR-004`](../architecture/ADR-004-five-product-modules-and-billing-events.md) is **target architecture direction**, not a v1 release gate — [`platform-architecture-principles.md`](../architecture/platform-architecture-principles.md) § Target architecture vs release criteria.
