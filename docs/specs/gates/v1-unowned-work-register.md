...
| ~~Identity tables have no RLS policy, and authentication depends on that~~ **Decided and closed 2026-08-29** — `users` and `user_memberships` carry policies; authentication reaches them over one named owner connection (`backend/app/auth/identity_session.py`), which is why those two are among the 15 tables deliberately not forced. `tenants` has no `tenant_id` and is not tenant-scoped | [tenant isolation](../tasks/tenant-isolation-enforcement.md) § TI-4 | Security canon owner | Closed |
