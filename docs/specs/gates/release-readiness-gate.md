...
Two conditions make even that band provisional: the [unowned work register](v1-unowned-work-register.md) § D4 must be emptied before the gate may open, and RC condition 4 is **half-satisfied** — migrations were measured on 2026-08-31 to apply cleanly to a freshly created database, and CI has been proving the same on every push ([§ Correction](../tasks/operate-and-launch.md)); what is missing is the condition's other clause, "by the documented procedure", since RB-2 does not exist, CI's Postgres major version differs from production's, and a freshly migrated instance still comes up with no admin user. Both are [Operate & Launch](../tasks/operate-and-launch.md) OL-2's. The earlier statement that this condition was *unsatisfiable* is withdrawn. A date announced without a queue roll-up behind it is not a release date.
