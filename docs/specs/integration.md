...
- **Админка:** `Админка → Лиды` позволяет:
  - указать/ротационировать `META_WEBHOOK_SECRET`, access token, `ad_account_id`, `page_id`; в таблице подключений видны **полные** Page ID и ad account ID (не только last4); Disable/Delete снимают страницу с тенанта;
...
- **Правила:**  
  - **POST** `/api/v1/leads/meta/webhook`: тенант только по `page_id` из события и активной строке `meta_lead_credentials` (владелец Page Access Token и app secret). Если в payload есть `page_id`, но ни один тенант эту страницу не подключил — **403**, без fallback на `webhook_verify_token` (иначе чужая Page, например Work Host, падала бы в Focus). `webhook_verify_token` в query остаётся для GET subscribe и для событий без `page_id`. При дубликате только verify token в `meta_lead_settings` приоритет у не‑legacy тенанта. Чтобы Poltrakt шёл в Focus, перенесите credentials на Focus и держите один владелец токена (см. `scripts/meta_poltrakt_to_focus_personnel.sql`).
  - **GET** `/api/v1/settings/leads/meta/forms` (Intake route picker): только формы, которые этот тенант уже заявил как Meta intake source, и только если форма всё ещё на подключённой Page (credential `page_id` или Graph этой Page). Leftover-маппинги и лиды с Page, которую тенант больше не держит, в список не попадают. Connect Source / Sources по-прежнему показывают Graph-каталог подключённых Pages, чтобы форму можно было заявить.  
