...
**Architecture:** **CLOSED** on `feat/eso4-formalize` @ `61cbc3eb` — [`recruitment-architecture-closed.md`](../specs/gates/recruitment-architecture-closed.md). Do not reopen for polish/cleanup; return only for closed-contract defects or proven handoff-boundary gaps.  
...
- **Входит:** Applications (отклики — inbound + Fits), кандидаты (Recruitment process → Ready for employment → **Передать на трудоустройство**), вакансии / подборы, пайплайн подбора, документы и правила стадий в контексте найма, handoff к HR/Fleet по продуктовым сценариям; хосты — [`ADR-042`](../specs/architecture/ADR-042-operator-host-boundary.md); **Vacancy Recruitment Requirements** (роль + что должно быть истинно; система: fit / missing / not_fit; **Подходит** остаётся человеческой границей) — [`vacancy-recruitment-requirements-sot.md`](../specs/tasks/vacancy-recruitment-requirements-sot.md); **Job Post / vacancy-facing publishing** как recruitment-specific surface — см. [`ADR-008`](../specs/architecture/ADR-008-job-publishing-and-distribution.md). **Кампании / рекламные кабинеты / универсальный intake routing** — Shared **Acquisition** ([`ADR-024`](../specs/architecture/ADR-024-acquisition-campaigns-intake-routing.md)); Подбор остаётся SoT потребности, не кампании. **Vacancy под Sales Order Line** — исполнитель одной линии заказа ([`ADR-032`](../specs/architecture/ADR-032-client-order-vacancy-flight-chain.md)); коммерция и headcount SoT — Sales Order Line, не Vacancy.
- **Не входит:** Sales Inquiry / ClientAccount / Service Order / Order Line / Billable Item (→ **Sales** / **Finance**); Service Order каталога услуг (→ **Services**); Invoice / Payment (→ **Finance**); **Employee Workspace**, Formalize, confirm start, Started и кадровый lifecycle (→ **HR** — Recruitment только handoff + ссылка, не Application chrome; Formalize→Started = HR scope, не Recruitment debt); операции автопарка (→ **Fleet**); владение Campaign/Ad как SoT (→ **Acquisition**). См. [`ADR-023`](../specs/architecture/ADR-023-recruitment-sales-module-separation.md). Full product spine после Transfer остаётся **NOT PASS / BLOCKED BY HR UI** — это не держит Recruitment открытым.
