...
- **Не входит:** Sales Inquiry / ClientAccount / Service Order / Order Line / Billable Item (→ **Sales** / **Finance**); Service Order каталога услуг (→ **Services**); Invoice / Payment (→ **Finance**); **Employee Workspace**, Formalize, confirm start, Started и кадровый lifecycle (→ **HR** — Recruitment только handoff + ссылка, не Application chrome); операции автопарка (→ **Fleet**); владение Campaign/Ad как SoT (→ **Acquisition**). См. [`ADR-023`](../specs/architecture/ADR-023-recruitment-sales-module-separation.md).
