...
- **`Lead`** — только внутренний transport intake; в UI модуля — **Отклик (Application)** / **Кандидат**, никогда «лид» как рабочий объект ([`ui-constitution-v1.md`](../specs/architecture/ui-constitution-v1.md)). Переход на карточку удалённого кандидата показывает явное empty-state; если отклик остался — действие «создать повторно» (см. [`candidates.md`](../specs/modules/candidates.md)).
