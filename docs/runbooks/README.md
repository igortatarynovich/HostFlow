...
| **RB-2** | Apply migrations to a fresh and to an existing database | RR3, RC condition 4 | igortatarynovich | [OL-2](../specs/tasks/operate-and-launch.md) | **MISSING** — but the underlying blocker is gone: `alembic upgrade heads` applies to a fresh DB in one command (measured twice, 2026-08-31 — [§ Correction](../specs/tasks/operate-and-launch.md)). CI already proves it too (`backend-ci.yml` job `alembic`, fresh `postgres:15`, green). What is missing is the **written procedure** itself — which is what RC condition 4's "by the documented procedure" requires — plus production parity (CI is on Postgres 15, production on 16) and a first-admin step: a freshly migrated instance starts with **no admin user** |
