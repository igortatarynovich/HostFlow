...
| **RB-2** | Apply migrations to a fresh and to an existing database | RR3, RC condition 4 | igortatarynovich | [OL-2B](../specs/tasks/operate-and-launch.md) | **DRAFT** — [rb-2-migrate-and-bootstrap.md](rb-2-migrate-and-bootstrap.md) written from the first throwaway pass. Migrate on Postgres 16 works; first-admin still FAIL; release compose did not start on this host (disk). Gate execution log empty |
| **RB-3** | Roll back a release | RR3, RR7 | igortatarynovich | [OL-2](../specs/tasks/operate-and-launch.md) | **MISSING** — no rollback procedure anywhere in the repo |
...
- 2026-08-31 (later, OL-2B): RB-2 moves MISSING → **DRAFT**. First throwaway pass on the production host built release artefacts, then STOP'd compose (`initdb` / no space). Sidecar Postgres 16 migrated to head; bootstrap admin still rejected. See [rb-2-migrate-and-bootstrap.md](rb-2-migrate-and-bootstrap.md). Not an execution record.
