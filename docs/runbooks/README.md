...
| **RB-2** | Apply migrations to a fresh and to an existing database | RR3, RC condition 4 | igortatarynovich | [OL-2](../specs/tasks/operate-and-launch.md) · [OL-2C](../specs/tasks/operate-launch-ol2c-ci-parity.md) | **MISSING** as an executed runbook. The CI/RB-2 proof-path is `scripts/deploy/release-proof.sh` (PG16-alpine, repo-root `alembic.ini`). First-admin and `/build` assertions wait on #336 / #332 |
