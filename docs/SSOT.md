...
- **Левый rail (IA):** **`hostflow-frontend/src/components/nav/Sidebar.tsx`** — секции с подписями (**`app.shell.sidebar.section_*`**, i18n **en/ru/pl**). Агентский primary rail — только production-ready поверхности: дашборд, inbox, рекрутинг, маркетинг (один хаб), продажи, настройки, профиль. Недозрелые модули (HR, услуги, финансы, документы-хаб, автоматизации, команда/календарь и вложенный маркетинг) скрыты из рейла, маршруты остаются. Список ключей, скрытых из рейла (но остающихся в **`NAV_ITEMS`**): **`hostflow-frontend/src/nav/appShellNav.ts`** (**`APP_SHELL_SIDEBAR_HIDDEN_ITEM_KEYS`**); состав рейла — **`sidebarRailBuckets.ts`**. Детальные экраны — через **`SettingsChrome`** / лендинг. **Topbar → меню пользователя:** те же зоны (**Личное** / **Организация** при **`companies.view`** / **Настройки** при **`settings.view`** / выход) — **`Topbar.tsx`**.
