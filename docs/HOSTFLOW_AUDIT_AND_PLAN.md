...
- **Frontend:** десятки страниц в `hostflow-frontend/src/pages/*`. Крупнейшие: ~~`Candidates.tsx` ~5.7k LOC~~ → 3008 LOC после Phase 1 #4, ~~`Dashboard.tsx` ~4.1k LOC~~ → **1210 LOC** (−71 %, −2933 строки) после Phase 1 #5 (14 извлечённых модулей: stageNormalize, internal, useDashboardKpiLoaders, useDashboardRiskOps, useDashboardRetention, useDashboardLayoutPrefs, useDashboardDerivedAnalytics, DashboardFiltersBar, DashboardExecutiveOverview, DashboardPivotPanels, DashboardAnalysisPanel, DashboardRetentionCenter, DashboardOpsOverviewPanels, DashboardPortfolioPanels). tsc + eslint + `npm run build` clean.
...
- [x] **Phase 1 #5 DONE — `hostflow-frontend/src/pages/Dashboard.tsx` (4143 LOC) разбит на пакет хуков/модулей** (9/9 шагов закрыто, 4143 → **1210 LOC**, −71 %, −2933 LOC). tsc + eslint + `npm run build` clean (production-bundle собран за 27 сек). Извлечённые модули:
    ...
    - **Шаг 7/N (DashboardAnalysisPanel, 234 LOC):** 178-строчный JSX-блок analytics-funnel + breakdown-table + problems-grid вынесен в отдельный компонент `src/modules/dashboard/components/DashboardAnalysisPanel.tsx`. Пропы: `funnelSteps`, `breakdownGroup/setBreakdownGroup`, `breakdownGroupOptions`, `breakdownMetric/setBreakdownMetric`, `breakdownRows`, `opsCounters`, `documentBlockerAnalytics`. **2639 → 2221 (−418, в сумме с ранее извлечёнными FiltersBar/ExecutiveOverview/PivotPanels за этот шаг).** (Фактические intermediate-цифры: до 2221 — см. summary ниже.)
    - **Шаг 8/N (DashboardRetentionCenter, 244 LOC):** 185-строчный JSX-кластер (retention-nudge баннер + billing-gate warning + trial center-секция с днями до конца / retention-report-таблицей) вынесен в `src/modules/dashboard/components/DashboardRetentionCenter.tsx`. Пропы: `canManageBilling`, `isTrialTenant`, `billingGate`, `retentionNudge`, `dismissRetentionNudge`, `trackRetentionEvent`, `showTrialPanel`, `trialCenterClasses`, `trialDaysLeft`, `trialTone`, `retentionReport(+Loading, +Rows)`. Исправлен импорт `TrialRetentionReport` (`api/client` → `api/analytics`). **2221 → 1890 (−331 за 2 шага: включая предыдущий DashboardAnalysisPanel).**
    - **Шаг 9a/N (DashboardOpsOverviewPanels, 428 LOC):** самый большой JSX-блок (~333 LOC) — triple-widgets (handoff/contact/documents) + stage-stack visual + error/range-invalid banners + global KPI-grid + business-profile cards + grouped stages & rejected/declined reasons — вынесен в `src/modules/dashboard/components/DashboardOpsOverviewPanels.tsx`. Пропы: `locale`, `formatNumber`, `makeCandidatesHref`, `documentQuickFilterHref`, `drilldownTitle`, `isWidgetVisible`, `handoffStats`, `contactStats`, `documentStats`, `documentBlockerAnalytics`, `profileSummary`, `stageStackSegments`, `errText`, `rangeInvalid`, `globalCounts`, `periodTotal`, `dateField`, `dashboardCompanyLabels`, `businessProfileCards`, `businessTypeLabel`, `businessCardHref`, `groupedStages`, `groupedRejectedReasons`, `groupedDeclinedReasons`, `drillInlineClass`. **1890 → 1586 (−304).**
    - **Шаг 9b/N (DashboardPortfolioPanels, 425 LOC):** финальный JSX-кластер (~393 LOC) — companies & vacancies таблицы, sources×stage detail, docs-risk-gauge, stage-velocity heat-bars, manager-load таблица, country-heatmap grid — вынесен в `src/modules/dashboard/components/DashboardPortfolioPanels.tsx`. Пропы: `formatNumber`, `makeCandidatesHref`, `drilldownTitle`, `drillInlineClass`, `isWidgetVisible`, `slices`, `stageLabels`, `dashboardCompanyLabels`, `sourceStageRows`, `docStageStats`, `stageVelocityRows`, `managerLoadRows`, `countryHeatmapRows`. **1586 → 1210 (−376).**

  **Итог Phase 1 #5 (9/9 закрыто):** `Dashboard.tsx` 4143 → 1210 LOC (−2933, −71 %); **16 извлечённых модулей** общим объёмом 265 + 30 + 200 + 340 + 380 + 169 + 589 + 420 + 129 + 254 + 234 + 244 + 428 + 425 = **4107 LOC**; каждый модуль 129–589 LOC (в среднем ~260). `tsc --noEmit` 0 ошибок, `eslint --quiet` clean, `npm run build` success (~27 сек). Orchestrating page = 1210 LOC: ~15 state-declarations + 14 hook-вызовов + JSX-skeleton с 7 сгруппированными компонентами верхнего уровня. Читается линейно.
