...
export const FAQ_SECTIONS: FaqSection[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: { en: "Add company → follow “Do this now” (client and/or vacancy depending on your type) → connect Meta ads or skip → get an application (ads, example data, or manual) → contact the person. The checklist always shows one next step. No ads yet — load example data.", ru: "Укажите компанию → «Сделайте сейчас» (клиент и/или вакансия по типу) → реклама Meta или пропуск → заявка (реклама, учебные данные или вручную) → связь с человеком. Чек-лист всегда показывает один шаг. Нет рекламы — загрузите учебные данные.", pl: "Podaj firmę → „Zrób teraz” (klient i/lub wakat wg typu) → reklamy Meta lub pomiń → zgłoszenie (reklamy, dane przykładowe lub ręcznie) → kontakt. Checklista zawsze pokazuje jeden krok. Brak reklam — wczytaj dane przykładowe." },
      },
      {
        id: "no_support",
        q: { en: "Can I start without talking to sales or support?", ru: "Можно ли начать без поддержки и отдела продаж?", pl: "Czy mogę zacząć bez supportu i sprzedaży?" },
        a: { en: "Yes. Signup, company setup, vacancy, and Meta connect are designed so you reach first results without a call.", ru: "Да. Регистрация, компания, вакансия и подключение Meta рассчитаны на самостоятельный старт без звонка.", pl: "Tak. Rejestracja, firma, wakat i Meta są zaprojektowane pod start bez rozmowy." },
      },
      {
        id: "interactive_demo",
        q: { en: "Is there an interactive demo?", ru: "Есть ли интерактивное демо?", pl: "Czy jest interaktywne demo?" },
        a: { en: "Yes. Open /demo, create your account, then “Load example data” on Start working. You explore your own company space and can clear the pack in one click — there is no shared guest login.", ru: "Да. Откройте /demo, создайте аккаунт и нажмите «Загрузить учебные данные» на экране «Начать работу». Вы работаете в своём аккаунте и можете очистить пакет одним кликом — общего гостевого входа нет.", pl: "Tak. Otwórz /demo, utwórz konto i kliknij „Wczytaj dane przykładowe” na ekranie „Zacznij pracę”. Pracujesz we własnej przestrzeni i możesz wyczyścić pakiet jednym kliknięciem — bez wspólnego loginu gościa." },
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: {
          en: "Recruitment agency — you hire for clients. Employer / transport / hospitality — you hire for your own company. Cleaning / services — clients and service requests come first (not the candidate list as home).",
          ru: "Кадровое агентство — нанимаете для клиентов. Работодатель / транспорт / HoReCa — нанимаете для своей компании. Клининг / услуги — сначала клиенты и заявки (не список кандидатов как главная).",
          pl: "Agencja rekrutacyjna — rekrutujesz dla klientów. Employer / transport / HoReCa — dla własnej firmy. Cleaning / usługi — najpierw klienci i zapytania (nie lista kandydatów jako home).",
        },
      },
      {
        id: "stuck_no_next",
        ...
        a: {
          en: "Open “Start working” and press the highlighted button. Empty lists always have a main action. If Meta ads are not ready, skip them or load example data. Still stuck? Load example data, then open Applications.",
          ru: "Откройте «Начать работу» и нажмите подсвеченную кнопку. У пустых списков всегда есть главное действие. Реклама Meta не готова — пропустите или загрузите учебные данные. Всё ещё тупик — учебные данные → Открыть заявки.",
          pl: "Otwórz „Zacznij pracę” i kliknij podświetlony przycisk. Puste listy zawsze mają główną akcję. Meta niegotowa — pomiń lub wczytaj dane przykładowe. Nadal tut — dane przykładowe → Otwórz zgłoszenia.",
        },
      },
      {
        id: "no_meta_ads",
        ...
        a: {
          en: "You can start without ads. On the checklist choose “Skip for now” for Meta, or on the applications screen pick Example data / Manual. Connect Meta later from Settings → Integrations when ready.",
          ru: "Можно начать без рекламы. В чек-листе — «Пока пропустить» для Meta, или на экране заявок — «Учебные данные» / «Вручную». Meta подключите позже: Настройки → Интеграции.",
          pl: "Możesz zacząć bez reklam. Na checkliście — „Pomiń na razie” dla Meta, albo na ekranie zgłoszeń — „Dane przykładowe” / „Ręcznie”. Meta podłącz później: Ustawienia → Integracje.",
        },
      },
      {
        ...
        q: {
          ...
          ru: "Meta не подключается или просит проверку приложения",
          pl: "Meta nie łączy się lub prosi o weryfikację aplikacji",
        },
        a: {
          en: "Usually missing Business admin approval, wrong ad account, or plan limits. Skip Meta for now, load example data to learn the product, then retry with a Business admin. Details: FAQ → Meta section.",
          ru: "Чаще нет одобрения администратора Business, неверный рекламный кабинет или лимиты тарифа. Пропустите Meta, загрузите учебные данные, повторите с администратором Business. Детали: FAQ → Meta.",
          pl: "Zwykle brak akceptacji admina Business, złe konto reklam lub limity planu. Pomiń Meta, wczytaj dane przykładowe, spróbuj z adminem Business. Szczegóły: FAQ → Meta.",
        },
      },
      {
        id: "no_leads",
        q: {
          en: "Why aren’t applications arriving?",
          ru: "Почему не приходят заявки?",
          pl: "Dlaczego nie przychodzą zgłoszenia?",
        },
        a: {
          en: "You need a vacancy (or a client for services), a way applications arrive (Meta ads / form / manual), and an active source. To unblock learning: Start working → Load example data → open Applications and contact the example person.",
          ru: "Нужны вакансия (или клиент для услуг), способ получения заявок (Meta / форма / вручную) и активный источник. Чтобы не ждать: Начать работу → Загрузить учебные данные → Заявки → связаться с примером.",
          pl: "Potrzebny wakat (lub klient dla usług), sposób zbierania zgłoszeń (Meta / formularz / ręcznie) i aktywne źródło. Bez czekania: Zacznij pracę → Wczytaj dane przykładowe → Zgłoszenia → kontakt z przykładem.",
        },
      },
      {
        id: "sample_data",
        q: {
          en: "What is example data and is it safe?",
          ru: "Что такое учебные данные и это безопасно?",
          pl: "Czym są dane przykładowe i czy to bezpieczne?",
        },
        a: {
          en: "A marked demo pack in your own company space (applications, candidates, tasks). It is not shared with other buyers. Clear it anytime with “Clear example data” before real hiring.",
          ru: "Помеченный учебный пакет в вашем аккаунте (заявки, кандидаты, задачи). Не общий с другими покупателями. Удалите в любой момент кнопкой «Удалить учебные данные» перед реальной работой.",
          pl: "Oznaczony pakiet demo we własnej przestrzeni (zgłoszenia, kandydaci, zadania). Nie jest współdzielony. Usuń w każdej chwili przyciskiem „Usuń dane przykładowe” przed realną pracą.",
        },
      },
      {
        id: "solo_no_team",
        ...
        a: {
          en: "Yes. Inviting teammates is optional. Skip invite on the checklist and work alone; add people later in Settings → Users.",
          ru: "Да. Приглашение коллег необязательно. Пропустите этот шаг в чек-листе и работайте один; людей добавите позже в Настройки → Пользователи.",
          pl: "Tak. Zaproszenie zespołu jest opcjonalne. Pomiń ten krok na checkliście i pracuj sam; ludzi dodasz później w Ustawienia → Użytkownicy.",
        },
      },
      {
