...
/** D5 communication slot — public Communication API only; not a second Inbox. */
export function ClientCommunicationSlot({ companyId }: Props) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="text-sm text-slate-600">
        {loading
          ? t('app.clients.communication.loading', { defaultValue: 'Загрузка переписки…' })
          : newest
            ? t('app.clients.communication.linked', {
                defaultValue: 'Переписка по клиенту',
              })
            : t('app.clients.communication.empty', {
      ...
      {newest ? (
        ...
        >
          {t('app.clients.communication.open_thread', { defaultValue: 'Открыть в Inbox' })}
        </Link>
      ) : (
        <Link to={buildInboxHubPath()} className="text-sm font-medium text-brand-700 hover:underline">
          {t('app.clients.communication.open_inbox', { defaultValue: 'Открыть Inbox' })}
        </Link>
      )}
    </div>
  )
