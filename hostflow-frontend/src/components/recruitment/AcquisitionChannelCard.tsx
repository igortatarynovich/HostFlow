...
import {
  acquisitionActivityStatusLabel,
  acquisitionNextActionTitle,
} from '../../utils/searchWorkspaceI18n'
...
export function AcquisitionChannelCard({ channel, metaHref }: AcquisitionChannelCardProps) {
  ...
  const nextActionTitle = acquisitionNextActionTitle(channel.next_action, t)
  const body = (
    <article className={`rounded-xl border p-4 shadow-sm ${statusTone(channel.status)}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 inline-flex items-center gap-1 text-sm text-slate-600">
            ...
            {acquisitionActivityStatusLabel(channel, t)}
          </p>
        </div>
      </div>
      ...
      {(channel.type === 'meta' || channel.type === 'public_link') && (
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-white/80 px-3 py-2 ring-1 ring-slate-100">
            <p className="text-xs text-slate-500">{t('app.acquisition.metric_responses')}</p>
            <p className="text-lg font-semibold text-slate-900">{responses}</p>
          </div>
          {channel.type === 'meta' ? (
            <>
              <div className="rounded-xl bg-white/80 px-3 py-2 ring-1 ring-slate-100">
                <p className="text-xs text-slate-500">{t('app.acquisition.metric_spend')}</p>
      ...
      {funnel.leads != null && channel.type === 'meta' ? (
        <div className="mt-4 rounded-xl bg-white/70 px-4 py-3 text-sm text-slate-700 ring-1 ring-slate-100">
          <p className="font-medium text-slate-900">{t('app.acquisition.funnel_title')}</p>
          <p className="mt-2 leading-relaxed">
            {funnel.leads ?? 0} {t('app.acquisition.funnel_leads')} →{' '}
            {funnel.candidates ?? 0} {t('app.acquisition.funnel_candidates')} →{' '}
            {funnel.interviews ?? 0} {t('app.acquisition.funnel_interviews')} →{' '}
            {funnel.offers ?? 0} {t('app.acquisition.funnel_offers')} →{' '}
            {funnel.hired ?? 0} {t('app.acquisition.funnel_hired')}
            {funnel.cost_per_hire != null ? (
              <span className="mt-1 block text-slate-600">
                {t('app.acquisition.cost_per_hire', { values: { amount: funnel.cost_per_hire } })}
              </span>
            ) : null}
          </p>
        </div>
      ...
      {nextActionTitle ? (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
          <span className="font-medium">{t('app.acquisition.next_action')}:</span>{' '}
          {nextActionTitle}
        </div>
      ) : null}
