...
/** D6 communication slot — public Communication API only; fold via company. */
export function SalesOrderCommunicationSlot({ companyId }: Props) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="text-sm text-slate-600">
        {loading
          ? t('app.sales_orders.communication.loading', { defaultValue: 'Загрузка переписки…' })
          : newest
            ? t('app.sales_orders.communication.linked', {
                defaultValue: 'Переписка по клиенту заказа',
              })
            : t('app.sales_orders.communication.empty', {
      ...
      {newest ? (
        ...
        >
          {t('app.sales_orders.communication.open_thread', { defaultValue: 'Открыть в Inbox' })}
        </Link>
      ) : (
        <Link to={buildInboxHubPath()} className="text-sm font-medium text-brand-700 hover:underline">
          {t('app.sales_orders.communication.open_inbox', { defaultValue: 'Открыть Inbox' })}
        </Link>
      )}
    </div>
  )
