...
/** D8 communication slot — public Communication API only; fold via candidate. */
export function HrEmployeeCommunicationSlot({ candidateId }: Props) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="text-sm text-slate-600">
        {loading
          ? t('app.hr.employee_detail.communication.loading', { defaultValue: 'Загрузка переписки…' })
          : newest
            ? t('app.hr.employee_detail.communication.linked', {
                defaultValue: 'Переписка по связанному кандидату',
              })
            : t('app.hr.employee_detail.communication.empty', {
      ...
      {newest ? (
        ...
        >
          {t('app.hr.employee_detail.communication.open_thread', { defaultValue: 'Открыть в Inbox' })}
        </Link>
      ) : (
        <Link to={buildInboxHubPath()} className="text-sm font-medium text-brand-700 hover:underline">
          {t('app.hr.employee_detail.communication.open_inbox', { defaultValue: 'Открыть Inbox' })}
        </Link>
      )}
    </div>
  )
