    ...
    <PageShell>
      ...
      <details className="group mx-4 mb-2 shrink-0 rounded-xl border border-slate-200/90 bg-white shadow-sm">
        <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
          {t('app.invoices.snapshot.title', { defaultValue: 'Billing snapshot' })}
          <span className="text-xs font-normal text-slate-500">
            · {formatAmount(invoiceSnapshot.totalOutstanding, '0')} · {invoiceSnapshot.overdue}{' '}
            {t('app.invoices.snapshot.overdue', { defaultValue: 'overdue' }).toLowerCase()}
          </span>
          <IconChevronDown size={16} className="ml-auto transition-transform group-open:rotate-180" aria-hidden />
        </summary>
              ...
              {activeFilterChips.map((chip) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => clearFilter(chip.key as 'company_id' | 'service_order_id' | 'status')}
                  >
        ...
        <DataTableFrame
          header={
            selectedIds.length > 0 ? (
              <div className="flex flex-wrap items-center gap-2 border-b border-slate-200/80 px-4 py-3 text-sm text-slate-700">
                <span>
          ...
          preScroll={
            error || actionError || actionMessage ? (
              <div className="space-y-2 border-b border-slate-200/80 px-4 py-3">
                {error ? (
                  <ErrorRecoveryBanner
                    info={error}
          ...
          footer={
            t('app.invoices.pagination.shown', {
              defaultValue: 'Showing {{count}} of {{total}}',
              values: { count: visibleInvoices.length, total: invoices.length },
            })
          }
        >
          {loading ? (
            <div className="px-4 py-8 text-center text-slate-500">{t('common.loading')}</div>
