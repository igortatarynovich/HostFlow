...
export default function CompanyIntakePage() {
  ...
  const candidateCountryOptions: Option[] = ['Ukraine', 'India', 'Georgia', 'Moldova', 'Belarus', 'Philippines'].map((value) => ({ value, label: value }))
  const scheduleOptions: Option[] = ['4/1', '6/2', '8/2', '2/1', 'Monday-Friday'].map((value) => ({ value, label: value }))
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await submitCompanyIntake(publicToken, {
        ...
        terms: {
          rate: optionalText(form.terms.rate),
          schedule: optionalText(form.terms.schedule),
        ...
        source_context: { ...sourceContext, language: form.language },
      })
    ...
    switch (currentStep) {
      ...
      case 'additional':
        return (
          <div className="space-y-5">
            <div>
              <p className="mb-3 text-sm font-medium text-slate-700">{t('public.company_intake.fields.candidate_countries', { defaultValue: 'Candidate countries' })}</p>
            ...
            <div>
              <p className="mb-3 text-sm font-medium text-slate-700">{t('public.company_intake.fields.schedule', { defaultValue: 'Schedule' })}</p>
              <ChoiceGrid options={scheduleOptions} value={form.terms.schedule} onChange={(value) => update('terms', 'schedule', value)} />
            </div>
            <div>
            ...
            <Field label={t('public.company_intake.fields.rate', { defaultValue: 'Rate' })}>
              <input className={inputClass} value={form.terms.rate} onChange={(e) => update('terms', 'rate', e.target.value)} placeholder="100 EUR/day" />
            </Field>
      ...
      <form className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-7" onSubmit={handleSubmit}>
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-slate-900">
            {t('public.company_intake.title', { defaultValue: 'Transport company questionnaire' })}
          </h1>
