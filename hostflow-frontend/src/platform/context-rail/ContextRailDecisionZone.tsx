...
function DecisionActionButton({
  ...
  const cls =
    size === 'primary'
      ? 'w-full justify-center rounded-lg bg-brand-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-800 disabled:opacity-60'
      : size === 'danger'
        ? 'rounded-lg border border-rose-200 bg-white px-2.5 py-1 text-xs font-medium text-rose-700 hover:bg-rose-50 disabled:opacity-60'
        : 'rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60'
  if (action.href) {
...
export function ContextRailDecisionZone({ decision }: { decision: ObjectDecision }) {
  ...
  if (decision.terminal && decision.outcome) {
    ...
    return (
      ...
      >
        ...
        <p className="mt-1 text-sm font-semibold leading-tight text-slate-900">{o.title}</p>
        ...
        {o.why ? (
          <div className="mt-1.5 border-l-2 border-slate-300 pl-2">
            ...
            <p className="mt-0.5 text-xs text-slate-700">{o.why}</p>
          </div>
        ) : null}
        {decision.why && !o.why ? (
          <div className="mt-1.5 border-l-2 border-slate-300 pl-2">
            ...
            <p className="mt-0.5 text-xs text-slate-700">{decision.why}</p>
          </div>
        ) : null}
  ...
  return (
    <div
      ...
      <p className="mt-1 text-sm font-semibold leading-tight text-slate-900">{decision.currentState}</p>
      {decision.why ? (
        <div className="mt-1.5 border-l-2 border-slate-300 pl-2">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
            {t('app.platform.context_rail.why')}
      ...
      {decision.afterActionHint ? (
        <p className="mt-1.5 text-xs text-slate-500">{decision.afterActionHint}</p>
      ) : null}
      {decision.secondaryActions?.length ? (
