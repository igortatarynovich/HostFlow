...
export function isDynamicImportChunkFailure(reason: unknown): boolean {
  const msg =
...
export function attemptStaleChunkReload(reason: unknown): boolean {
  if (typeof window === 'undefined') return false
  if (!isDynamicImportChunkFailure(reason)) return false
...
export function installStaleChunkReloadRecovery(): void {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    attemptStaleChunkReload(event.reason)
  })
}
