...
def test_eso5_confirm_without_employee_is_blocked_no_mint() -> None:
    out = apply_employment_started_v1(
        package=_pkg(),
        handoff_status="accepted",
        ready_to_create_employee=True,
...
def test_eso5_confirm_without_start_allowed_is_blocked() -> None:
    out = apply_employment_started_v1(
        package=_pkg(),
        handoff_status="accepted",
        employee_id="emp-1",
...
def test_eso5_replay_does_not_emit_second_event() -> None:
    ...
    first = apply_employment_started_v1(
        ...
        prior_start=prior,
        start_confirmation={"confirmed": True},
        start_allowed=False,  # already started — admit not re-required
    )
