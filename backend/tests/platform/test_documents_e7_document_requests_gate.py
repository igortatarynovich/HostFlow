"""Documents Platform E7 — Document Requests Gate.

Outstanding-ask SoT = Hub required type + entity via Document Link,
consumed as `outstanding_asks` on `documents.hub_adapter_v1`.
D4 + D8 stay bound. D3 / D5–D7 / D9 stay unbound.
Candidate stage / HR JSON / Activity `document_request` are not Documents SoT.
No Hub request / reminder / task table. Same adapter; no new public-contract id.
...
def test_e7_brief_locks_hub_outstanding_ask() -> None:
    text = _BRIEF.read_text(encoding="utf-8")
    assert "Documents Platform E7" in text
    assert "Document Requests" in text or "outstanding" in text.lower()
    assert "outstanding" in text.lower()
    assert "document.requested" in text
    assert "Documents Platform E7 Document Requests Gate" in text
    assert "OCR" in text
    assert "hr_document_requests" in text
    assert "document_request" in text


def test_e7_d4_and_d8_stay_bound_others_omit() -> None:
...
def test_e7_public_consume_is_hub_outstanding_ask() -> None:
    ...
    assert "project_outstanding_asks_via_contract" in delivery
    assert "outstanding_asks" in delivery
    assert "outstanding_asks" in api
    assert "OutstandingAskOut" in api
    assert "outstandingAsks" in owner or "outstanding_asks" in owner
    assert "data-outstanding-ask" in capability
    assert "outstanding_asks" in contract
...
def test_e7_proof_surface_uses_capability_host_and_hub_adapter() -> None:
...
def test_e7_shell_documents_nav_is_not_d2_slot() -> None:
...
def test_e7_g4_recruitment_application_unchanged() -> None:
...
def test_e7_maturity_documents_foundation_not_complete() -> None:
    ...
    assert "E7" in foundation_cell or "Phase E" in foundation_cell


def test_e7_catalog_shape_unchanged_no_ocr_product() -> None:
...
def test_e7_e6_closed_and_prior_gates_present() -> None:
    e6 = _E6_BRIEF.read_text(encoding="utf-8")
    assert "**COMPLETE**" in e6 or "#285" in e6
    ...
    assert "test_documents_e7_document_requests_gate.py" in ci
    ...
    assert "documents-platform-e7-document-requests.md" in hub


def test_e7_product_track_points_at_e7() -> None:
    queue = (
        _REPO_ROOT
        / "docs"
        / "specs"
    ...
    assert "documents-platform-e7-document-requests.md" in queue
    assert "documents-platform-e7-document-requests.md" in agents
    assert "request" in agents.lower()


def test_e7_gate_filename() -> None:
    assert Path(__file__).name == "test_documents_e7_document_requests_gate.py"
