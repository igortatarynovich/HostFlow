"""Queue amendment names FP-1 Active Product.

Mapping program DONE. Active Product = FP-1 (brief; feat locked).
Forms Publish Contract Gate not PASS. Do not seal the publish contract
in this PR. Hiring / min HR remain queued. Leftover-store deletion
and RS-3 stay unauthorized.
"""
...
def test_fp1_named_active_feat_locked() -> None:
    ...
    assert "Forms Publish Contract Gate not PASS" in current
    assert "Active (Product):** **[FP-1](external-intake-forms-publish.md)**" in current
    assert "**Active Product** | **DONE**" not in current
    assert "Do not open FP-1 contract seal in this PR" in current
    ...
    assert "Forms Publish Contract Gate **not PASS**" in intake_current
    assert "Do not open FP-1 contract seal" in intake_current or "does **not** seal the publish contract" in intake_current
