"""Entity Workspace D6 — Cutover Gate.

Consumer = Sales Order (ADR-032). D2 catalog unchanged.
Reserved `documents` cannot be enabled. Shell nav ≠ composition slots.
No Catalog Passport. No HR / Vacancy / Services /app/orders cutover.
D1 + D2 + D3 + D4 + D5 gates remain. No Postgres required.
"""
...
_COMM_SLOT = (
    ...
    / "components"
    / "sales"
    / "SalesOrderCommunicationSlot.tsx"
)
_FORMS_SLOT = (
    _REPO_ROOT
    / "hostflow-frontend"
...
def test_d6_brief_locks_sales_order_consumer() -> None:
    text = _BRIEF.read_text(encoding="utf-8")
    assert "Sales Order Cutover" in text
    assert "Entity Workspace D6 Cutover Gate" in text
    assert "Catalog Passport" in text
    assert "documents" in text
    assert "SalesOrderDetailPage" in text
    assert "service_order" in text
    assert "/app/sales/orders" in text


def test_d6_sales_order_binding_matches_enabled_catalog() -> None:
    src = _CONSUMER_TS.read_text(encoding="utf-8")
    slots = _ts_string_array(src, "SALES_ORDER_COMPOSITION_SLOTS")
    assert slots == _EXPECTED_CONSUMER_SLOTS
    assert "documents" not in slots
    assert "SALES_ORDER_COMPOSITION_CONSUMER_ID" in src
    assert "sales-order" in src
    assert "assertSalesOrderCompositionSlots" in src
...
def test_d6_page_composes_d2_slots() -> None:
    page = _PAGE.read_text(encoding="utf-8")
    host = _HOST_TSX.read_text(encoding="utf-8")
    assert "EntityWorkspaceCompositionHost" in page
    assert "SALES_ORDER_COMPOSITION_SLOTS" in page
    assert "SalesOrderCommunicationSlot" in page
    assert "SalesOrderFormsSlot" in page
...
def test_d6_px_mock_order_is_not_this_consumer() -> None:
    mocks = _MOCKS_TS.read_text(encoding="utf-8")
    consumer = _CONSUMER_TS.read_text(encoding="utf-8")
    assert "MOCK_ORDER_MODEL" in mocks
    assert "service_orders" in mocks
    assert "MOCK_ORDER_MODEL" not in consumer
    assert "resourceId: 'service_orders'" not in consumer


def test_d6_services_order_page_not_cut_over() -> None:
    text = _SERVICES_PAGE.read_text(encoding="utf-8")
    assert "SALES_ORDER_COMPOSITION_SLOTS" not in text
    assert "salesOrderConsumer" not in text
    assert "assertSalesOrderCompositionSlots" not in text


def test_d6_shell_sections_not_collapsed() -> None:
...
def test_d6_hr_vacancy_services_not_cut_over() -> None:
    leaked: list[str] = []
    needles = (
        "SALES_ORDER_COMPOSITION_SLOTS",
        "salesOrderConsumer",
    ...
    for path in sorted(_FRONTEND_SRC.rglob("*.ts")) + sorted(_FRONTEND_SRC.rglob("*.tsx")):
        ...
        lowered = rel.lower()
        if not any(token in lowered for token in ("hr", "vacancy", "services")):
            continue
        text = path.read_text(encoding="utf-8")
        if any(needle in text for needle in needles):
            leaked.append(rel)
    assert not leaked, f"D6 must not cut over HR/Vacancy/Services: {leaked}"


def test_d6_no_entity_catalog_passport_mint() -> None:
...
def test_d6_maturity_entity_foundation_not_complete() -> None:
...
def test_d6_prior_gates_still_present() -> None:
    assert _D5_BRIEF.is_file()
    ...
    assert "test_entity_workspace_d6_cutover_gate.py" in ci


def test_d6_product_track_points_at_brief() -> None:
    queue = (
        _REPO_ROOT
        / "docs"
        / "specs"
    ...
    assert "entity-workspace-d6-sales-order-cutover.md" in queue
...
def test_d6_gate_filename() -> None:
    assert Path(__file__).name == "test_entity_workspace_d6_cutover_gate.py"
