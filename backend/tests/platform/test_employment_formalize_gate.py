...
Reference evaluate stays mint-free; orchestrator mints only on authoritative apply
via ensure_employee_after_formalize_apply. No HR card. No universal checklist. LLM-OFF.
"""
...
def test_eso4_orchestrator_wires_ensure_on_authoritative_apply_only() -> None:
    ...
    # Mint only via ensure seam (handoff_from_candidate), not direct workforce create.
    assert "workforce_employees" not in orch
    tree = ast.parse(orch)
    funcs = {n.name for n in tree.body if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef))}
    assert "formalize_employment_for_handoff" in funcs
    assert "formalize_request_is_authoritative_apply" in funcs
