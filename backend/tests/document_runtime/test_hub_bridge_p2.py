...
def test_p2_hub_checklist_exposes_document_runtime_v1() -> None:
    hub = _hub_for([_approved("passport")])
    ...
    passport = _item(hub, "passport")
    assert passport["document_runtime"]["evaluation_version"] == DOCUMENT_RUNTIME_V1
    assert passport["lifecycle_status"] == "approved"


def test_p2_approved_valid_satisfied() -> None:
    future = (date.today() + timedelta(days=120)).isoformat()
    hub = _hub_for([_approved("passport", expires_on=future)])
    item = _item(hub, "passport")
...
def test_p2_uploaded_not_satisfied() -> None:
    hub = _hub_for([{"type": "passport", "status": "uploaded", "has_files": True}])
    item = _item(hub, "passport")
...
def test_p2_pending_review_not_satisfied() -> None:
    hub = _hub_for([{"type": "passport", "status": "submitted", "has_files": True}])
    item = _item(hub, "passport")
    assert item["satisfies_requirement"] is False
...
def test_p2_rejected_blocker() -> None:
    hub = _hub_for([{"type": "passport", "status": "rejected", "has_files": True}])
    item = _item(hub, "passport")
    assert item["satisfies_requirement"] is False
...
def test_p2_expired_blocker() -> None:
    ...
    hub = _hub_for([_approved("passport", expires_on=past)])
    item = _item(hub, "passport")
    assert item["satisfies_requirement"] is False
    assert item["status"] == "problem"
...
def test_p2_expiring_soon_warning_but_satisfied() -> None:
    ...
    hub = _hub_for([_approved("passport", expires_on=soon)])
    item = _item(hub, "passport")
    assert item["satisfies_requirement"] is True
    assert item["status"] == "satisfied"
...
def test_p2_best_instance_wins_over_weaker_duplicate() -> None:
    documents = [
        {"type": "passport", "status": "uploaded", "has_files": True, "document_id": "weak"},
        {"type": "passport", "status": "approved", "has_files": True, "document_id": "strong"},
    ]
    evaluation = evaluate_requirement_rules(
        _profile_view_from_manifest(),
        context="readiness",
    ...
    passport = next(row for row in checklist["items"] if row["document_type_code"] == "passport")
    assert passport["document_id"] == "strong"
    assert passport["satisfies_requirement"] is True
...
def test_p2_merge_owner_summary_includes_runtime_checklist() -> None:
    ...
    assert "passport" in merged["required"]["in_progress_types"]


def test_p2_apply_hub_requirements_to_checklist() -> None:
    hub = _hub_for([_approved("passport"), _approved("driver_license"), _approved("code95"), _approved("tacho_card")])
    checklist = apply_hub_requirements_to_checklist({"requiredTypes": []}, hub)
    assert checklist["requiredTypes"] == ["passport", "driver_license", "code95", "tacho_card"]
    assert len(checklist["runtimeItems"]) == 4
