...
def evaluate_employment_started_v1(
    *,
    ...
    Slice 4 (PEM-1): Employee must already exist; first Confirm requires
    ``start_allowed=true``. Mint-on-confirm is retired.
    """
    ...
    # Slice 4: Employee must exist before Confirm (no mint-on-confirm).
    can_mint = False
    if not employee_exists:
        blockers.append(
            {
                "code": "employee_required",
                "message": "Started requires an existing handoff-linked Employee (Formalize→ensure); Confirm must not mint",
            }
        )

    # Slice 4: first physical Confirm requires admit-to-work.
    if (
        not already
        and confirm.get("confirmed")
        and employee_exists
        and start_allowed is not True
    ):
        blockers.append(
            {
                "code": "start_allowed_required",
                "message": "Confirm physical start requires start_allowed=true (employment_start_allowed.v1)",
            }
        )

    active_missing: list[dict[str, str]] = []
    if not already and not reuse_violations and employee_exists:
        if start_allowed is not True:
            active_missing.append(
                {
                    "code": "start_allowed",
                    "kind": "admit",
                    "message": "Admit-to-work (start_allowed) is not true yet",
                }
            )
        if not start_date:
            active_missing.append(
    ...
    admit_block = any(b.get("code") == "start_allowed_required" for b in blockers)
    if reuse_violations or conflict or (not employee_exists) or admit_block:
        decision = DECISION_BLOCKED
        primary = {
            "code": "resolve_preconditions",
            "kind": "blocker",
            "message": "Resolve Employee / start_allowed / existing start conflict",
        }
        started = False
        emitted = False
        replay = False
    ...
    mint_employee = False
    employee_created = employee_exists
    ...
    return {
        ...
        "start_allowed": True if start_allowed is True else False if start_allowed is False else None,
        "started": started,
