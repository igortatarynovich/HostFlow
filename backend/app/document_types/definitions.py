...
DRIVER_DOCUMENT_TYPES: Tuple[DocumentTypeDefinition, ...] = (
    ...
    DocumentTypeDefinition(
        ...
        expiry_rule=_expiry_rule("expires_at"),
        default_expire_in_days=1825,
        # Registry/pack/R5 name driver_qualification_card is NOT a second Hub type —
        # it is an inbound alias of this storage code (Accepted DQC catalog↔pack bridge).
        aliases=(
    ...
    DocumentTypeDefinition(
        ...
        kind=DocumentKind.employer,
        requested_from=DocumentRequestedFrom.employer,
        canonical_ref_code="employment_contract",
        metadata_schema=_object_schema(
            ("employer_name", "signed_at", "start_at"),
            {
                "employer_name": {"type": "string"},
                "signed_at": _date_schema(),
                "start_at": _date_schema(),
                "end_at": _date_schema(),
                "contract_number": {"type": "string"},
                "position": {"type": "string"},
                "written_instrument_confirmed": {"type": "boolean"},
                "instrument_kind": {"type": "string"},
                "employer_id": {"type": "string"},
                "confirmation_of_terms": {"type": "boolean"},
                "confirmed_at": _date_schema(),
            },
        ),
        required_files=_any_required(min_files=1, preset="a4", frame_kwargs=_frame_kwargs(A4_FRAME)),
        duplicate_policy=DocumentDuplicatePolicy.many_allowed,
        aliases=("contract", "umowa_o_prace"),
        owner_summary_weight=80,
    ),
    DocumentTypeDefinition(
        code="bhp",
        ...
        kind=DocumentKind.employer,
        requested_from=DocumentRequestedFrom.employer,
        canonical_ref_code="bhp",
        metadata_schema=_object_schema(
            ("training_date",),
            {
                "training_date": _date_schema(),
                "training_kind": {"type": "string"},
                "training_type": {"type": "string"},
                "provider": {"type": "string"},
                "employer_id": {"type": "string"},
                "post_key": {"type": "string"},
                "applies_to_post": {"type": "string"},
                "expires_at": _date_schema(),
            },
        ),
        required_files=_any_required(min_files=1, preset="a4", frame_kwargs=_frame_kwargs(A4_FRAME)),
