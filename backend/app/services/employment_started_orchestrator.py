...
Slice 4 (PEM-1): Confirm requires ``start_allowed=true``; does not mint Employee.
"""
...
async def _resolve_handoff_employee(
    db: AsyncSession,
    *,
    tenant_id: str,
    handoff: CandidateHandoff,
) -> WorkforceEmployee | None:
    """Prefer Employee linked to this handoff (ESA2), else candidate-scoped lookup."""
    agency = _text(getattr(handoff, "agency_tenant_id", None)) or str(tenant_id)
    candidate_id = _text(getattr(handoff, "candidate_id", None))
    if not candidate_id:
        return None
    for tid in {str(tenant_id), agency}:
        emp = await we_svc.find_employee_by_candidate(db, tid, candidate_id)
        if emp is None:
            continue
        linked = employee_linked_handoff_id(emp)
        if linked and linked == str(handoff.id):
            return emp
    # Fallback: same candidate employee (legacy rows without linkage stamp)
    for tid in {str(tenant_id), agency}:
        emp = await we_svc.find_employee_by_candidate(db, tid, candidate_id)
        if emp is not None:
            return emp
...
async def confirm_employment_started_for_handoff(
    ...
    *,
    ...
    Slice 4: refuses mint-on-confirm; requires start_allowed=true for first Started.
    """
    if ensure_employee:
        raise EmploymentStartedError(
            "mint_on_confirm_forbidden",
            "Confirm must not mint Employee; complete Formalize→ensure first",
        )
    ...
    employee = await _resolve_handoff_employee(db, tenant_id=str(tenant_id), handoff=handoff)
    ...
    # Admit-to-work: reuse ESA-3 host evaluate (derived; never operator-set).
    admit = await evaluate_start_allowed_for_handoff(
        db,
        tenant_id=str(tenant_id),
        handoff_id=str(handoff.id),
        employment_context=ctx or None,
    )
    start_allowed = bool(admit.get("start_allowed"))
    ...
    result["start_allowed"] = start_allowed
    result["mint_employee"] = False

    if employee is not None:
        result["employee_id"] = str(employee.id)
        result["employee_created"] = True
    else:
        result["employee_created"] = False
