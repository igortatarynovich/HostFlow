...
async def resolve_candidate_acl(
    ...
    if is_team_lead_org_actor(role):
    ...
    operational_employee = trust == TrustRole.employee.value or role in (
    ...
    if operational_employee or portal_or_viewer:
        ...
        manager_scope = (
            {user_id}
            if role == Role.recruiter.value or (trust == TrustRole.employee.value and role != Role.compliance_officer.value)
            else set()
        )
        return CandidateACL.restricted(
            company_ids=company_ids,
