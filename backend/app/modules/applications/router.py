...
async def list_recruitment_applications(
    ...
    result = await service.list_leads(
        ...
        tenant_id=str(tenant_id),
        own_company_id=own_company_id,
        recruitment_inbox_tab=inbox_tab,
        recruitment_inbox_scope=inbox_scope,
        recruitment_vacancy_id=vacancy_filter,
        recruitment_order_updated_at=(inbox_tab == "completed"),
        limit=limit,
        offset=offset,
    )
    items = [lead_to_recruitment_application(row) for row in result.items]
