...
def upgrade() -> None:
    ...
    if _has_table(conn, "tenants"):
        if not _has_column(conn, "tenants", "type"):
            ...
            op.execute(
                sa.text("UPDATE tenants SET type = :value WHERE type IS NULL").bindparams(value="agency")
            )
        ...
        if not _has_column(conn, "tenants", "status"):
            ...
            op.execute(
                sa.text("UPDATE tenants SET status = :value WHERE status IS NULL").bindparams(value="active")
            )
        ...
        if not _has_column(conn, "tenants", "workspace_label"):
            op.add_column(
                "tenants",
                sa.Column("workspace_label", sa.String(length=128), nullable=True),
            )
            op.execute(sa.text("UPDATE tenants SET workspace_label = name WHERE workspace_label IS NULL"))

        if not _has_column(conn, "tenants", "logo_url"):
            op.add_column(
                "tenants",
                sa.Column("logo_url", sa.String(length=512), nullable=True),
            )

        if not _has_column(conn, "tenants", "logo_meta"):
            op.add_column(
                "tenants",
                sa.Column("logo_meta", sa.JSON(), nullable=True),
            )
...
def downgrade() -> None:
    ...
    if _has_table(conn, "tenants"):
        ...
        if _has_column(conn, "tenants", "logo_meta"):
            op.drop_column("tenants", "logo_meta")
        if _has_column(conn, "tenants", "logo_url"):
            op.drop_column("tenants", "logo_url")
        if _has_column(conn, "tenants", "workspace_label"):
            op.drop_column("tenants", "workspace_label")
