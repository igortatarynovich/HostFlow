...
**Release close-out (mandatory):** «готовы к запуску» объявляет **только** [`release-readiness-gate.md`](docs/specs/gates/release-readiness-gate.md) — семь вопросов, entry conditions, определение Release Candidate, выводимая дата RC. Доказательство — [`release-readiness-acceptance-suite.md`](docs/specs/journeys/release-readiness-acceptance-suite.md) (RS-1…RS-12, прогоняет не разработчик). Пустая очередь, закрытая программа или зелёный named gate — **не** release-ready.

**Launch-ops (второй трек):** [`operate-and-launch.md`](docs/specs/tasks/operate-and-launch.md) — v1 blocker 6 (deploy / rollback / backup / restore / tenant lifecycle / support). Обязательный набор процедур и их статус — [`docs/runbooks/README.md`](docs/runbooks/README.md) (RB-2 DRAFT; остальные MISSING). Контракт жизненного цикла данных тенанта — [`ADR-039`](docs/specs/architecture/ADR-039-tenant-data-lifecycle.md) (Proposed): export / erasure / retention — platform capability, модули только регистрируют participant-ы; soft delete ≠ erasure. **Миграции (исправлено 2026-08-31):** `alembic upgrade heads` на чистой БД **проходит**. Bootstrap-админ закрыт в-graph (`202608310001_bootstrap_admin_schema`); OL-2B остаётся **NOT EXECUTED** до независимого witness.

Near-term slices: [`docs/specs/tasks/sales-to-comms-sequential-queue.md`](docs/specs/tasks/sales-to-comms-sequential-queue.md) — **Product Track** = **[HE-4](docs/specs/tasks/hiring-workflow-e2e.md)** (feat `feat/hiring-e2e-he4-acceptance-walk` open from `8d5a9fef`; Hiring E2E Acceptance Gate **not PASS**; RS-7 not executed; Eligibility Composition Gate **PASS**; SoT [hiring-eligibility-composition.md](docs/specs/architecture/hiring-eligibility-composition.md); `hiring_eligibility_composition.v1`; parent [hiring-acceptance-contract.md](docs/specs/architecture/hiring-acceptance-contract.md); Stage Authority Consumption Gate **PASS**; Hiring Acceptance Contract Gate **PASS**) after External Intake program close recorded ([external-intake-forms-publish.md](docs/specs/tasks/external-intake-forms-publish.md)) (Publish Action Gate **PASS**; External Intake Acceptance Gate **PASS**; feat `feat/forms-publish-fp5-runtime`; Operator Publish Gate **PASS**) after Forms Publish Contract Gate **PASS** ([forms-publish-contract.md](docs/specs/architecture/forms-publish-contract.md); `forms_publish.v1`) after Mapping program close recorded ([mapping-authority.md](docs/specs/tasks/mapping-authority.md); Mapping Consumer Cutover Gate **PASS** `fddadd39`; evidence `92206f40`) after Mapping Operator Gate **PASS** ([mapping-authority-operator.md](docs/specs/architecture/mapping-authority-operator.md)) after Mapping Resolution Gate **PASS** ([mapping-authority-resolution.md](docs/specs/architecture/mapping-authority-resolution.md); `resolve_mapping_authority`) after Mapping Authority Contract Gate **PASS** ([mapping-authority-contract.md](docs/specs/architecture/mapping-authority-contract.md); `mapping_authority.v1`) after RPM program **DONE** ([requirement-policy-management.md](docs/specs/tasks/requirement-policy-management.md); Consumer Cutover Gate **PASS** `918274d1` / RPM-3B Consumer Parity Gate **PASS** / RPM-3A Parallel Authority Retirement Gate **PASS** / RPM-2 Operator Gate **PASS** [#342](https://github.com/igortatarynovich/HostFlow/pull/342) / `5196ee64` / RPM-1 Authority Gate **PASS** ([requirement-policy-authority.md](docs/specs/architecture/requirement-policy-authority.md)) / E8-eval Gate **PASS** [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6` ([documents-platform-e8-eval.md](docs/specs/tasks/documents-platform-e8-eval.md)) and DAG review [#328](https://github.com/igortatarynovich/HostFlow/pull/328)). UX SoT: [mapping-authority-operator.md](docs/specs/architecture/mapping-authority-operator.md). UX contract Accepted; Mapping Operator Gate PASS; Mapping Consumer Cutover Gate **PASS** (`fddadd39`); Mapping program **DONE**. **v1 in-scope vs later:** [`hostflow-v1-release-goal.md`](docs/specs/gates/hostflow-v1-release-goal.md). External Intake **FP-2** Gate PASS; **FP-3** feat opened this PR (`feat/forms-publish-fp3-public-serve` open; Public Serve Gate **PASS**). min HR not auto-scheduled. **Engineering Track** = **DONE** — Reference Program Exit Gate PASS [#298](https://github.com/igortatarynovich/HostFlow/pull/298) / `ff0b914c` ([platform-reference-identity-sot.md](docs/specs/tasks/platform-reference-identity-sot.md); R1 **named Country Registry Gate** PASS [#292](https://github.com/igortatarynovich/HostFlow/pull/292); R5 **named Policy Merge Gate** PASS [#297](https://github.com/igortatarynovich/HostFlow/pull/297)). Sequence: [queue § Locked execution sequence](docs/specs/tasks/sales-to-comms-sequential-queue.md) — Product `CL0 → CL1 → LI-1 → DR1-contract → CL2… → Overlay → DR1-runtime → E8-bind → E8-eval → RPM-1 → RPM-2 → RPM-3A → RPM-3B` (**DONE**) → **MA-1** → **MA-2** → **MA-3** → **MA-4** → Mapping program close → **FP-1** (**PASS**) → **FP-2** (**PASS**) → **FP-3** → **FP-4** → **FP-5** → External Intake program close → **HE-1** → **HE-2** → **HE-3** → **HE-4**; Engineering `R1 → {R2 ∥ R3} → R4 → (R2 ∧ R4) → R5 → Program Exit` (**DONE**; no successor this amendment). E8-eval Gate **PASS** [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6` after E8-bind Gate PASS [#321](https://github.com/igortatarynovich/HostFlow/pull/321) / `8246421f`; OCR stays locked (unlock ≠ schedule). Always write **Reference R1** (not Epic C residual R1 / C2.4). **Queued (docs only):** [Lifecycle Identity](docs/specs/tasks/lifecycle-identity-l0-contract-seal.md) ([ADR-037](docs/specs/architecture/ADR-037-lifecycle-identity-canon.md); Funnel ≠ existence SoT). E8-bind ✅ ([#321](https://github.com/igortatarynovich/HostFlow/pull/321); [documents-platform-e8-bind.md](docs/specs/tasks/documents-platform-e8-bind.md); named Canonical Type Bind Gate). E7 ✅ ([#287](https://github.com/igortatarynovich/HostFlow/pull/287); [documents-platform-e7-document-requests.md](docs/specs/tasks/documents-platform-e7-document-requests.md); named Document Requests Gate). E6 ✅ ([#285](https://github.com/igortatarynovich/HostFlow/pull/285); [documents-platform-e6-document-expiry.md](docs/specs/tasks/documents-platform-e6-document-expiry.md); named Document Expiry Gate; expiry / validity). E3 ✅ ([#278](https://github.com/igortatarynovich/HostFlow/pull/278); [documents-platform-e3-first-consumer-bind.md](docs/specs/tasks/documents-platform-e3-first-consumer-bind.md); named First Consumer Bind Gate; first consumer = HR employee + Document Link SoT). E2 ✅ ([#276](https://github.com/igortatarynovich/HostFlow/pull/276); [documents-platform-e2-public-contract.md](docs/specs/tasks/documents-platform-e2-public-contract.md); named Public Contract Gate). [Workspace Capability Platform Completion](docs/specs/tasks/workspace-capability-platform-completion.md) is [COMPLETE](docs/specs/gates/workspace-capability-platform-complete.md) on [#274](https://github.com/igortatarynovich/HostFlow/pull/274); intermediate [G1–G5 PASS_WITH_CONSTRAINTS](docs/specs/gates/workspace-capability-platform-g1-g5-closeout.md) on [#273](https://github.com/igortatarynovich/HostFlow/pull/273). G4 PASS (Recruitment Application) — **not** the E3 proof. D1–D9 are brief-complete / **goal-incomplete** vs original D chrome ([audit](docs/specs/gates/platform-scope-completeness-audit.md)): [D1](docs/specs/tasks/entity-workspace-d1-contract-seal.md) · [D2](docs/specs/tasks/entity-workspace-d2-composition-contract.md) · [D3](docs/specs/tasks/entity-workspace-d3-consumer-cutover.md) · [D4](docs/specs/tasks/entity-workspace-d4-candidate-cutover.md) · [D5](docs/specs/tasks/entity-workspace-d5-client-cutover.md) · [D6](docs/specs/tasks/entity-workspace-d6-sales-order-cutover.md) · [D7](docs/specs/tasks/entity-workspace-d7-vacancy-cutover.md) · [D8](docs/specs/tasks/entity-workspace-d8-hr-employee-cutover.md) · [D9](docs/specs/tasks/entity-workspace-d9-services-order-cutover.md) (named Cutover Gate). E1 ✅ ([#270](https://github.com/igortatarynovich/HostFlow/pull/270); [documents-platform-e1-contract-seal.md](docs/specs/tasks/documents-platform-e1-contract-seal.md); named Contract Seal Gate). Close-out gate: [`goal-completion-gate.md`](docs/specs/gates/goal-completion-gate.md). E2 brief ✅ ([#271](https://github.com/igortatarynovich/HostFlow/pull/271)). Catalog unlock ≠ consumer bind. E3 ✅ binds **one** consumer (HR employee). E4 ✅ binds Candidate via [Document Link](docs/specs/tasks/documents-platform-e4-candidate-document-link.md). E5 drops `candidate_id` ([documents-platform-e5-candidate-storage-bridge.md](docs/specs/tasks/documents-platform-e5-candidate-storage-bridge.md)). E6 seals Hub expiry / validity. D3 / D5–D7 / D9 stay unbound. Not D10. Not a Recruitment rail patch. Not ListWorkspace. Stage 5 settings, R6, Forms P4–P5, OCR, packages, and mass D3–D9 `documents` bind are **not** this slice. Forms **Publish** Action Gate **PASS**; External Intake Acceptance Gate **PASS**; feat `feat/forms-publish-fp5-runtime`; P4 / P5 stay later.
...
**Active close-out:** [Workspace Capability Platform Completion](docs/specs/tasks/workspace-capability-platform-completion.md) [COMPLETE](docs/specs/gates/workspace-capability-platform-complete.md) → [host runtime-equivalence](docs/specs/tasks/workspace-capability-host-runtime-equivalence.md) ✅ → … → Documents E7 ✅ [#287](https://github.com/igortatarynovich/HostFlow/pull/287) → Overlay ✅ [#311](https://github.com/igortatarynovich/HostFlow/pull/311) → DR1-runtime ✅ [#313](https://github.com/igortatarynovich/HostFlow/pull/313) → E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321) / `8246421f` → E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6` ← **Product = [HE-4](docs/specs/tasks/hiring-workflow-e2e.md)** (feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**; Eligibility Composition Gate **PASS**) after External Intake program close ([external-intake-forms-publish.md](docs/specs/tasks/external-intake-forms-publish.md)) (Publish Action Gate **PASS**; External Intake Acceptance Gate **PASS**; feat `feat/forms-publish-fp5-runtime`; Operator Publish Gate **PASS**); Forms Publish Contract Gate PASS; Mapping program close recorded; Mapping Consumer Cutover Gate PASS; Mapping Operator Gate PASS; Mapping Resolution Gate PASS; MA-1 Contract Gate PASS; RPM program DONE) · [v1 Release Goal](docs/specs/gates/hostflow-v1-release-goal.md) ∥ **Reference program DONE** (Exit PASS [#298](https://github.com/igortatarynovich/HostFlow/pull/298) / `ff0b914c`)

**Locked:** Acquisition/Stage 3 (Phase B) ✅ → Forms Platform ✅ → Entity Workspace D1–D9 (brief-complete) → Workspace Capability Platform Completion (**COMPLETE** / G4 PASS) → [host runtime-equivalence](docs/specs/tasks/workspace-capability-host-runtime-equivalence.md) ✅ → Documents E2…E7 ✅ → Entity Field Composition CL0–CL7 / LI-1 / DR1-contract / Overlay / DR1-runtime ✅ → E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321) → E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) ← **Product = [HE-4](docs/specs/tasks/hiring-workflow-e2e.md)** (feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**; Eligibility Composition Gate **PASS**) after External Intake program close ([external-intake-forms-publish.md](docs/specs/tasks/external-intake-forms-publish.md)) (Publish Action Gate **PASS**; External Intake Acceptance Gate **PASS**; feat `feat/forms-publish-fp5-runtime`; Operator Publish Gate **PASS**); Forms Publish Contract Gate PASS; Mapping program close recorded; Mapping Consumer Cutover Gate PASS; Mapping Operator Gate PASS; Mapping Resolution Gate PASS; MA-1 Contract Gate PASS; RPM program DONE) · [v1 Release Goal](docs/specs/gates/hostflow-v1-release-goal.md) ∥ **Reference program DONE**; Engineering `R1 → {R2 ∥ R3} → R4 → (R2 ∧ R4) → R5 → Program Exit` (**DONE**); OCR / packages / automation plane / extensions / self-service Billing / AI = **later** (unlock ≠ schedule). External Intake **FP-2** Gate **PASS**; Product = **[HE-4](docs/specs/tasks/hiring-workflow-e2e.md)** (feat `feat/hiring-e2e-he4-acceptance-walk` open; Hiring E2E Acceptance Gate **not PASS**; Eligibility Composition Gate **PASS**) after Stage Authority Consumption Gate **PASS** ([external-intake-forms-publish.md](docs/specs/tasks/external-intake-forms-publish.md); feat `feat/forms-publish-fp5-runtime`; External Intake Acceptance Gate **PASS**; Operator Publish Gate **PASS**). min HR not auto-scheduled.  
...
### Migration caveats (non-obvious) — **re-measured 2026-08-31**
**`alembic upgrade heads` applies cleanly to a freshly created database.** Measured twice on empty
Postgres 16 containers on the current trusted base (`f03a4dbd`), once with `upgrade heads` and once with
`upgrade head`: both exit 0, apply all 299 revisions, land on
`202608250002_merge_e5_drop_and_adr036_heads` and produce 236 tables, with no manual step of any kind.

The three manual steps this section used to prescribe are obsolete, and the graph itself is why:

1. **Widening `alembic_version` by hand is unnecessary.** Alembic hardcodes `version_num VARCHAR(32)`
   (`DefaultImpl.version_table_impl`) and 109 of the 299 revision ids are longer than that, but
   `backend/alembic/versions/20260113_widen_alembic_version.py` widens the column to `VARCHAR(255)`
   inside the graph, before any long id has to be recorded.
   Note: the `version_table_pk_column` / `version_table_pk_column_type` arguments in
   `backend/alembic/env.py` are **dead** — `MigrationContext` reads only `version_table`,
   `version_table_schema` and `version_table_pk`, and silently discards the rest. Do not trust them.
2. **Hand-ordering the parallel branch tips is unnecessary.** The graph has one head and 25 merge
   points, and alembic's own ordering applies the siblings correctly.
3. **`202605200001` does not need to be stamped over.** It applies normally, and its successor
   `202511130001` drops the legacy scan-session objects as designed.

**Bootstrap admin (closed in-graph 2026-08-31, OL-2B implementation):** ADR-036 already persists
`superadmin` as a trust role; `users.preferences` is a required empty JSON object. The native
`role` enum and the column default did not match that contract. Both insert errors below were
real on a freshly migrated Postgres 16. They are fixed by
[`202608310001_bootstrap_admin_schema.py`](backend/alembic/versions/202608310001_bootstrap_admin_schema.py).
The seed writes `preferences` and no longer swallows a failed insert; `main.py` does not catch
`ensure_auth_seed`. `auth_seed_enabled()` stays off unless `HOSTFLOW_AUTH_SEED_ENABLED=1`. When
the seed is off, first admin is `POST /api/v1/auth/register` (role `administrator`). Do not
hand-run `ALTER TYPE` / `SET DEFAULT`.
