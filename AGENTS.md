...
**Launch-ops (второй трек):** [`operate-and-launch.md`](docs/specs/tasks/operate-and-launch.md) — v1 blocker 6 (deploy / rollback / backup / restore / tenant lifecycle / support). Обязательный набор процедур и их статус — [`docs/runbooks/README.md`](docs/runbooks/README.md) (сейчас все MISSING). Контракт жизненного цикла данных тенанта — [`ADR-039`](docs/specs/architecture/ADR-039-tenant-data-lifecycle.md) (Proposed): export / erasure / retention — platform capability, модули только регистрируют participant-ы; soft delete ≠ erasure. **Миграции:** `alembic upgrade heads` на чистой БД не проходит (см. § Migration caveats) — это владение **OL-2**, и это блокирует определение Release Candidate.
