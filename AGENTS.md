...
**Locked:** Acquisition/Stage 3 (Phase B) ✅ → Forms Platform ✅ → Entity Workspace D1–D9 (brief-complete) → Workspace Capability Platform Completion (**COMPLETE** / G4 PASS) → [host runtime-equivalence](docs/specs/tasks/workspace-capability-host-runtime-equivalence.md) ✅ → Documents E2…E7 ✅ → Entity Field Composition CL0 (brief; feat locked) ← **Product active** ∥ Reference R1 ← **Engineering active** → CL1 → LI-1 → DR1-contract → CL2…; Engineering `R1 → {R2 ∥ R3}`; E8-bind / E8-eval split → Billing → AI.  
