...
Near-term slices: [`docs/specs/tasks/sales-to-comms-sequential-queue.md`](docs/specs/tasks/sales-to-comms-sequential-queue.md) — **Platform Baseline v1** ([Kit Gate](docs/specs/gates/platform-extraction-kit-gate.md) · [Inventory](docs/specs/platform/platform-inventory.md)). **Active slice** = Forms Platform (Phase C). Meta [#222](https://github.com/igortatarynovich/HostFlow/pull/222) ✅ · slice 3 [#224](https://github.com/igortatarynovich/HostFlow/pull/224) ✅ · slice 4 [#238](https://github.com/igortatarynovich/HostFlow/pull/238) ✅. Stage 5 settings and R6 are **not** this slice.
...
**Active close-out:** [Forms Platform](docs/specs/architecture/platform-completion-roadmap.md) (Phase C) → Entity Workspace → Documents → Billing → AI. Phase B Meta/slice 3–4 ✅ ([#222](https://github.com/igortatarynovich/HostFlow/pull/222) · [#224](https://github.com/igortatarynovich/HostFlow/pull/224) · [#238](https://github.com/igortatarynovich/HostFlow/pull/238)).

**Locked:** Platform Extraction (kit gate ✅ · **Platform Baseline v1** · [Inventory](docs/specs/platform/platform-inventory.md)) → Acquisition/Stage 3 (Phase B) ✅ → Forms Platform ← **active** → Entity Workspace → Documents → Billing → AI.  
...
4. **Один чат — одна логическая задача. Не копить длинный контекст.**
   - Один чат = один concern / один slice / один PR-sized кусок работы. Не продолжать «ещё чуть-чуть» в том же чате, когда логическая задача уже закрыта.
   - Когда задача завершена (результат сдан, следующий шаг — другая задача или новый срез), агент **обязан явно сказать**: логическая задача закрыта, дальше лучше новый чат.
   - В том же сообщении дать **handoff-блок** — самодостаточный контекст для следующего агента, чтобы тот не искал всё заново по репозиторию и истории.
   - Не предлагать новый чат посреди незакрытой работы (отладка, незакоммиченный diff, ждущий аппрув план). Предлагать — на границе задач.
   - Если пользователь явно просит продолжить здесь — продолжать, но при следующем логическом закрытии снова дать handoff.

   **Handoff (обязательный шаблон в конце закрывающего сообщения):**

   ```
   ## Чат можно закрывать

   Логическая задача завершена. Следующий шаг — в новом чате (вставь блок ниже первым сообщением).

   ### Что сделано
   - …

   ### Где смотреть (не искать заново)
   - Ветка / PR:
   - Файлы:
   - Коммиты / спеки:
