...
**Active close-out:** [Workspace Capability Platform Completion](docs/specs/tasks/workspace-capability-platform-completion.md) [COMPLETE](docs/specs/gates/workspace-capability-platform-complete.md) → [host runtime-equivalence](docs/specs/tasks/workspace-capability-host-runtime-equivalence.md) ✅ → … → Documents E7 ✅ [#287](https://github.com/igortatarynovich/HostFlow/pull/287) → Overlay ✅ [#311](https://github.com/igortatarynovich/HostFlow/pull/311) → DR1-runtime ✅ [#313](https://github.com/igortatarynovich/HostFlow/pull/313) → E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321) / `8246421f` → E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) / `19c95ef6` ← **Product = [RPM-1](docs/specs/tasks/requirement-policy-management.md)** (brief; feat locked) · [v1 Release Goal](docs/specs/gates/hostflow-v1-release-goal.md) ∥ **Reference program DONE** (Exit PASS [#298](https://github.com/igortatarynovich/HostFlow/pull/298) / `ff0b914c`)

**Locked:** Acquisition/Stage 3 (Phase B) ✅ → Forms Platform ✅ → Entity Workspace D1–D9 (brief-complete) → Workspace Capability Platform Completion (**COMPLETE** / G4 PASS) → [host runtime-equivalence](docs/specs/tasks/workspace-capability-host-runtime-equivalence.md) ✅ → Documents E2…E7 ✅ → Entity Field Composition CL0–CL7 / LI-1 / DR1-contract / Overlay / DR1-runtime ✅ → E8-bind ✅ [#321](https://github.com/igortatarynovich/HostFlow/pull/321) → E8-eval ✅ [#324](https://github.com/igortatarynovich/HostFlow/pull/324) ← **Product = [RPM-1](docs/specs/tasks/requirement-policy-management.md)** (brief; feat locked) · [v1 Release Goal](docs/specs/gates/hostflow-v1-release-goal.md) ∥ **Reference program DONE**; Engineering `R1 → {R2 ∥ R3} → R4 → (R2 ∧ R4) → R5 → Program Exit` (**DONE**); OCR / packages / automation plane / extensions / self-service Billing / AI = **later** (unlock ≠ schedule). Mapping / Hiring E2E not auto-scheduled.  
...
**What is still true, and is now the real fresh-database defect:** migrations succeed, but the schema
they produce cannot accept the admin the bootstrap writes. `backend/app/auth/ensure_seed.py` inserts
`role='superadmin'` and never supplies `preferences`, while no migration adds `superadmin` to the `role`
enum and `users.preferences` is `NOT NULL` with no server default. Both were reproduced directly on the
fresh database:
...
The seed catches this, prints `[seed] вставка admin не удалась` and lets startup continue, so a fresh
instance comes up **healthy with no admin user** — a silent failure, not a loud one. `ALTER TYPE role ADD
VALUE IF NOT EXISTS 'superadmin';` and `ALTER TABLE users ALTER COLUMN preferences SET DEFAULT
'{}'::jsonb;` still unblock it locally. The owning slice for a real fix is **OL-2**
([operate-and-launch.md](docs/specs/tasks/operate-and-launch.md)). Note also that
`auth_seed_enabled()` is off unless `HOSTFLOW_AUTH_SEED_ENABLED=1`, so on a production target this seed
